<template>
  <div class="index" @click="uploadImg=false;">
    ...
    <!-- 图片标题 -->
    <div class="div-title" @click="toIndex" style="cursor:pointer;" title="返回主页"><img :src="'./static/images/search/searchTitle'+ this.$config.project.localSuffix+'.png'"></div>
    <!-- 搜索框 -->
    <div class="wrapper">
      <div class="box">
        
        <div class="search-box">
          <i-button class="search-default" :disabled="!hasFunAuth(constantFunMap.FUN_Search_Data_ID)" @click="toRelationPage">搜索一下</i-button>
          <!-- <span class="search-btn" @click="toRelationPage">
            <span>搜索一下</span>
          </span> -->
          <div class="input">
            <!-- <i class="iconfont icon-tianjiazhaopian" @click="uploadImages"></i> -->
            <!-- <i class="iconfont icon-tianjiazhaopian" @click.stop="showUploadImage"></i>
            <i class="iconfont icon-yuyin"></i> -->
            <input type="text" :disabled="!hasFunAuth(constantFunMap.FUN_Search_Data_ID)" placeholder="请输入查询关键字"  v-model="keyword" @keyup.enter="toRelationPage"/>
          </div>
        </div>
      </div>
...
export default {
  ...
  methods : {
    ...
    showUploadImage() {
      // 查询权限
      if(!this.hasFunAuth(this.constantFunMap.FUN_Search_Data_ID)) {
