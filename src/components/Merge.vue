 ...
 * @LastEditTime: 2019-10-28 18:14:31
 * @LastEditors: your name
 * @Description: In User Settings Edit
 * @FilePath: \map-web\src\components\Merge.vue
 -->
<template>
    ...
    <div class="result-cont">
      ...
      <div id="d3" class="d3">
        <div class="title">
          ...
          <Input v-model="searchHighlightNode" placeholder="输入搜索条件" style="width: 300px;margin-left:50px;" @keyup.enter.native="searchHighlight"/>
          <Button type="primary" icon="ios-search" @click="searchHighlight">搜索</Button>
        </div>
        <!-- d3图形展示 -->
        <div class="graphical" v-show="statusList.d3Show">
          <!-- 风险要素组件 -->
          <riskLevel :assessData="assessData" @highlightNodes="highlightNodes"></riskLevel>
    ...
    <transition name="multiStr">
      <div class="multi-str" v-show="statusList.MultiModal">
        <div class="condition-modal">
          ...
          <div class="cont-btn">
            <span class="cancel" @click="closeMultiModal">取消合并</span>
            <span class="sure" @click="getTreeData(true)">开始合并</span>
...
export default {
  ...
  created () {
    ...
    //this.hydra = require(`../../static/vendors/hydraCard.js`);  //D3卡片展示
  ...
  methods : {
    ...
    searchHighlight () {
      var me = this;
      if(!me.searchHighlightNode){
        me.$Message.warning('请输入查询条件！');
        return false;
      }
      console.log(this.searchHighlightNode)
      var searchBoxText=this.searchHighlightNode;
      ...
      me.filterHighlightIds(me.tpData.children,me.resultNodes);
      //3 根据找到的ID进行高亮
      me.highlightNodeIDs.forEach(function(item){
    ...
    filterHighlightIds (data,resultNodes){
      ...
      data.forEach(function(item){
        ...
        for(var i in itemArr){
          if(itemArr[i]!='' && typeof(itemArr[i])!= 'number' && item.nodeType!='QB_ZP_TP'){
            // console.log(itemArr[i],994994);
            // console.log(me.resultLabel,995995);
            if(itemArr[i].indexOf(me.resultLabel) != -1){//存在这个Label
              ...
              me.resultId.push(item.id);
              console.log(item.oid,10031003);
              //点击标签实体颜色高亮
              //me.graph.showKeyNode(true,item.oid,'blue');
              //console.log(me.highlightNodeIDs,1007);
              //me.graph.highlightNodeIDs.push(item.oid);
              me.highlightNodeIDs.push(item);
              // if(me.prevId == item.oid){
              //   me.graph.showKeyNode(true,item.oid,'blue');
              // }else{
              //   me.graph.showKeyNode(true,item.oid,'#d7d7d7');
              // }
              ...
              // if(me.prevId == item.oid){
              //   if(me.nodesStatus){
              //     me.nodesStatus = false;
              //     me.graph.showKeyNode(me.nodesStatus,item.oid,'');
              //   }else{
              //     me.nodesStatus = true;
              //     me.graph.showKeyNode(me.nodesStatus,item.oid,'blue');
              //   }
              // }else{
              //   me.nodesStatus = true;
              //   me.graph.showKeyNode(me.nodesStatus,item.oid,'blue');
      ...
      // console.log(me.resultId,10141014);
      // me.parseData(me.tpData);
      // // const data = response.data.links;
      // // const linksData = me.mergeLinksData;
     ...
     */
    saveAsPng(){
      this.graph.exportPic(this.saveName + '合并图');
    },
    refresh(){
      let me = this;
    ...
    getJzmc (flag) {
      ...
      let url = me.joinPath + 'mapmergeresource/v1/map/keyword';
    ...
    getJzmcData (flag) {
      ...
      me.statusList.loadingTpmcShow = true;
      let url = me.joinPath + 'mapmergeresource/v1/map/region/type';
      var params = new URLSearchParams();
    ...
    getTreeData (flag) {
      ...
      var params = new URLSearchParams();
      let url = me.joinPath + 'mapmergeresource/v1/map/tree/id';
      params.append('token', me.token);
      ...
      .then(response => {
        ...
        console.log(me.commonIds,'AAAAAA');
        me.parseData(me.tpData,false);
        // const data = response.data.links;
    ...
    parseData(data, flag){
      ...
      function changeImage(data){
        ...
        // console.log(data.oid,'XXXXXXXX');
        // console.log(data.id,'AAAAAA');

        //图谱合并连线
        if(me.commonIds.length && me.commonIds.indexOf(data.id) != -1){
          // console.log(data.id,'YYYYYY');
          data.color = '#fd00fc';
        }
        //标签点击高亮对应实体
        if(me.resultId.length && me.resultId.indexOf(data.id) != -1){
          // console.log(data.id,'YYYYYY');
          data.color = '#fd00fc';
        }
