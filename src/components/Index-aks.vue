<template>
  ...
  >
    ...
    <app-header ref="appHeader" :items="statusList" title="综合门户" :showSearch="false"></app-header>
    <!-- 图标内容区 -->
    <div class="charts-box">
      ...
      <div class="charts charts-box-left">
        <div class="charts-box">
          ...
          <div class="charts-type">
            <div class="nav">
              <p class="charts-title">
                <span class="title-name" style="cursor:pointer;" @click="toArchivesJoin">{{ms}}</span>
              </p>
              <div class="title">
            ...
            <div class="charts-area">
              <div
                class="no-auth"
                v-if="!hasFunAuth(constantFunMap.FUN_Index_ZONG_JIAO_JI_DUAN_R_ID)"
              >{{ constantNoAuthData }}</div>
              <!-- echart -->
          ...
          <div class="charts-type">
            <div class="nav">
              <p class="charts-title">
                <span class="title-name" @click="goMapManagerByType('2')">{{this.$config.mapType.SWGW}}</span>
              </p>
              <div class="title">
                <i class="separate-icon"></i>
                <div class="rel-drop"></div>
              </div>
            </div>
            <div class="charts-area">
              <div
                class="no-auth"
                v-if="!hasFunAuth(constantFunMap.FUN_Index_JIA_ZU_R_ID)"
              >{{ constantNoAuthData }}</div>
              <charts class="charts-ele" ref="home" :ed="home" color="#262729" totalColor="#204A88"></charts>
            </div>
          </div>
          <!-- 左3 -->
          <div class="charts-type">
            <div class="nav">
              <p class="charts-title">
                <span class="title-name" @click="goMapManagerByType('9')">{{this.$config.mapType.SHGX}}</span>
              </p>
              <div class="title">
                <i class="separate-icon"></i>
              </div>
            ...
            <div class="page-box">
              <Page
                :total="society.allData.length"
                :page-size="14"
                :current="society.currentPage"
            ...
            <div class="charts-area">
              <div
                class="no-auth"
                v-if="!hasFunAuth(constantFunMap.FUN_Index_LI_NIAN_BEI_DA_JI_F_ID)"
              >{{ constantNoAuthData }}</div>
              <charts
                class="charts-ele"
                ref="society"
      ...
      <div class="charts charts-box-center">
        ...
        <div class="selected" v-if="this.$config.homeConfig.showTreeDept">
          <Dropdown trigger="click">
            <a href="javascript:void(0)">
              <span
                style="width:80px;"
              >{{ unitListData.filterDataLength != '' ? unitListData.filterDataLength : '请选取单位' }}</span>
              <Icon type="ios-arrow-down"></Icon>
            </a>
            <DropdownMenu slot="list">
              <Tree
        ...
        <div class="map">
          <div
            class="no-auth"
            v-if="!hasFunAuth(constantFunMap.FUN_Index_MAPDADIAN_ID)"
          >{{ constantNoAuthData }}</div>
          <chartsMap
            class="charts-map"
            ref="map"
      ...
      <div class="charts charts-box-right">
        <div class="charts-box">
          ...
          <div class="charts-type">
            <div class="nav">
              <p class="charts-title">
                <span class="title-name" @click="goMapManagerByType('4')">{{this.$config.mapType.AJGX}}</span>
              </p>
              <div class="title">
                <i class="separate-icon"></i>
              </div>
            ...
            <div class="charts-area">
              <div
                class="no-auth"
                v-if="!hasFunAuth(constantFunMap.FUN_Index_SHE_AN_REN_YUAN_AN_JIAN_R_ID)"
              >{{ constantNoAuthData }}</div>
              <charts
                class="charts-ele"
                ref="highrisk"
          ...
          <div class="charts-type">
            <div class="nav">
              <p class="charts-title">
                <span class="title-name" @click="goMapManagerByType('10')">{{this.$config.mapType.CRJ}}</span>
              </p>
              <div class="title">
                <i class="separate-icon"></i>
              </div>
            ...
            <div class="page-box">
              <Page
                :total="HajjData.RowBarData.length"
                :page-size="12"
                :current="HajjData.currentPage"
                @on-change="getPageRbd"
              />
            </div>
            <div class="charts-area">
              <div
                class="no-auth"
                v-if="!hasFunAuth(constantFunMap.FUN_Index_JING_WAI_SHE_BAO_R_ID) "
              >{{ constantNoAuthData}}</div>
              <charts
                class="charts-ele"
                ref="border"
          ...
          <div class="charts-type">
            <div class="nav">
              <p class="charts-title">
                <span class="title-name" @click="goMapManagerByType('8')">{{this.$config.mapType.SCRY}}</span>
              </p>
              <div class="title">
                <i class="separate-icon"></i>
              </div>
            ...
            <div class="charts-area">
              <div
                class="no-auth"
                v-if="!hasFunAuth(constantFunMap.FUN_Index_CHU_RU_JING_F_ID)"
              >{{ constantNoAuthData }}</div>
...
import chartsMap from "@/components/assembly/Map-aks";
...
export default {
  ...
  methods: {
    ...
    toArchivesJoin() {
      var me = this;
      if(!me.hasFunAuth(me.constantFunMap.FUN_Index_ZONG_JIAO_JI_DUAN_Map_View_ID)) {
        // me.$Message.warning(me.constantNoAuthData)
        return;
      }
      var name = "宗教极端思想来源";
    ...
    // 跳转到在线采集的图谱管理的页面
    goMapManagerByType(type) {
      let hasAuth = this.hasPageAuth(this.constantPageMap.PAGE_Online_MapManager_ID)
      if(hasAuth) {
        this.$router.push({
    ...
    toHeritage(id) {
      var me = this;
      if(!me.hasPageAuth(me.constantPageMap.PAGR_Heritage_ID)) {
        // me.$Message.warning(me.constantNoAuthData)
        return;
      }
      me.$router.push({
    ...
    getAllTpData() {
      let me = this;
      apis
        .queryMapPersonCount(me.unitListData.dwmc)
        .then(response => {
          if (response.code == 200) {
            var data = response.data.count;
            me.tpCount.tableData = data;
            //console.log(me.tpCount.tableSum,905905)
          } else {
    ...
    //   let me = this;
    //   apis
    //     .queryCountyCount(this.zongjiaoBar, this.unitListData.dwmc, "/zplx6")
    //     .then(response => {
    //       if (response.code == 200) {
    ...
    getRowBarData() {
      let me = this;
      apis
        .queryCountyCount(this.sichao, this.unitListData.dwmc, "/zplx7")
        .then(response => {
          if (response.code == 200) {
            var data = response.data.data;
            ...
            me.HajjData.rbd = me.HajjData.RowBarData.slice(
              (this.HajjData.currentPage - 1) * this.HajjData.pageSize,
              12
            );
            me.HajjData.rbdv = me.HajjData.RowBarDataVal.slice(
              (this.HajjData.currentPage - 1) * this.HajjData.pageSize,
              12
            );
            me.drawRowBar();
    ...
    getLineData(msg, type) {
      let me = this;
      apis
        .queryCountyCount(msg, this.unitListData.dwmc, type)
        .then(response => {
          if (response.code == 200) {
            var data = response.data;
    ...
    getChartsData(msg, type) {
      ...
      apis
        .queryCountyCount(msg, this.unitListData.dwmc, type)
        .then(response => {
          if (msg.chartType == "RowBar") {
            var data = response.data.data;
            me[msg.chartID].chartData = response.data.data;
            me.society.currentPage = 1;
            me.society.allData = me[msg.chartID].chartData;
            me[msg.chartID].chartData = me[msg.chartID].chartData.slice(
              (me.society.currentPage - 1) * me.society.pageSize,
              me.society.pageSize
            );
          } else {
            me[msg.chartID].chartData = response.data.data;
    ...
    getPageIndex(page) {
      ...
      var data = this.society.allData.slice(
        (this.society.currentPage - 1) * this.society.pageSize,
        this.society.currentPage * this.society.pageSize
      );
    ...
    getPageRbd(page) {
      ...
      this.HajjData.rbd = this.HajjData.RowBarData.slice(
        (this.HajjData.currentPage - 1) * this.HajjData.pageSize,
        this.HajjData.currentPage * this.HajjData.pageSize
      );
      this.HajjData.rbdv = this.HajjData.RowBarDataVal.slice(
        (this.HajjData.currentPage - 1) * this.HajjData.pageSize,
        this.HajjData.currentPage * this.HajjData.pageSize
      );
      this.drawRowBar();
    ...
    getMapData() {
      ...
      apis
        .queryothercountyperson(me.unitListData.dwmc)
        .then(response => {
          this.$nextTick(() => {
    ...
    toRelationPage() {
      // 查询权限
      if (!this.hasPageAuth(this.constantPageMap.PAGE_Search_ID)) {
    ...
    getInitData(){
      ...
      if (this.hasFunAuth(this.constantFunMap.FUN_Index_CHU_RU_JING_F_ID))
        this.getRowBarData();//zplx7，出入境
      if (this.hasFunAuth(this.constantFunMap.FUN_Index_JIA_ZU_R_ID))
        this.getLineData(this.home, "/zplx2");//家族
      if (this.hasFunAuth(this.constantFunMap.FUN_Index_LI_NIAN_BEI_DA_JI_F_ID))
        this.getChartsData(this.society, "/zplx5");//历年被打击处理人员分布示意图
      if (this.hasFunAuth(this.constantFunMap.FUN_Index_SHE_AN_REN_YUAN_AN_JIAN_R_ID))
        this.getChartsData(this.highrisk, "/zplx2");//涉案人员案件关系图
      if (this.hasFunAuth(this.constantFunMap.FUN_Index_JING_WAI_SHE_BAO_R_ID))
        this.getChartsData(this.border, "/zplx3");//境外涉恐涉暴人员网络关系图
  ...
  mounted() {
    var me = this;
    this.getInitData();
    if (this.hasFunAuth(this.constantFunMap.FUN_Index_MAPDADIAN_ID))
      this.getMapData();
  },
  created() {
...
.index {
  ...
  .inforArea-enter-active,
  .inforArea-leave-active {
    transition: all 0.5s;
  }
  .inforArea-enter,
  .inforArea-leave-active {
...
.cont-right::-webkit-scrollbar-thumb:hover {
  background-color: rgba(45, 183, 245, 1);
}
.cont-right::-webkit-scrollbar-corner {
...
.charts-type {
  ...
  .nav {
    ...
    .title {
      ...
      .post(a);
      //top:30px;
      background: url(../../static/images/chart/title-icon-aks.png) 0 0
        no-repeat;
  ...
  .selected {
    .ivu-dropdown {
      ...
      .ivu-select-dropdown {
        ...
        left: 0 !important;
        color: #fff;
        background-color: rgba(51, 171, 255, 0.6);
  ...
  .charts-type {
    .page-box {
      ...
      .ivu-page-prev {
        ...
        background: rgba(38, 156, 219, 0.5);
        a {
          color: #fff;
        }
      }
