 ...
 * @LastEditTime: 2019-12-12 18:06:16
    ...
    <div class="login-box">
      ...
      <div class="form">
        <div class="form-box">
          ...
          <ul>
            <li class="login-type">
              <span v-if="!$config.project.showOnlyPkiLogin" @click="toDefaultPage">密码登录</span>
              <span
                @click="toOauthPage"
                :style="{'width':$config.project.showOnlyPkiLogin ? '100%' : false}"
              >PKI 登录</span>
              <span v-if="!$config.project.showOnlyPkiLogin" @click="toFacePage">人脸/指纹登录</span>
            </li>
          </ul>
        </div>
      </div>
      <!-- 下载谷歌 -->
      <a class="Chrome-download" :href="$config.fileDownloadPath+'chromes/Chrome64.zip'" download>
        <img src="../../static/images/login/download-icon.png" alt />Chrome浏览器下载
      </a>
    </div>
  </div>
</template>
  ...
  methods: {
    ...
    toOauthPage() {
      ...
      this.$store.dispatch("getPkiClientAuthInfo").then(response => {
        let url = `${response.uiasUrl}uias/${response.onUri}?response_type=code&scope=read,write&client_id=${response.clientId}&state=${state}&redirect_uri=${response.redirectUri}`;
        console.log("url", url);
