          ...
          <div class="item-header">
            <div class="title" :key="index" v-for="(item,index) in data.nodeList">
              <div class="title-info">
                <span class="node-type" :style="nodeTypeStyle(item)">{{item.nodeType == "QB_ZP_TP" ? item.properties.mapTypeName : item.nodeType}}</span>
                <span @click="handleCaseClubManage(item)" class="node-name">{{item.nodeType == "QB_ZP_TP" ? item.properties.mapName : item.properties.name}}</span>
              </div>
              <div class="line"></div>
            </div>
...
export default {
  ...
  methods: {
    ...
    nodeTypeStyle(item) {
      const style = {};
      style.background = item.nodeType == "QB_ZP_TP" ? this.$config.topTagColors[item.properties.mapTypeName] : item.nodeType == '案件' ? 'rgb(215, 67, 72)' : 'rgb(66, 100, 215)';
      return style;
    },
    // 关闭事件
    handleClose() {
    ...
    handleCaseClubManage(item) {
      ...
      if (item.nodeType == "QB_ZP_TP") {
        let hasAuth = this.hasPageAuth( this.constantPageMap.PAGE_D3_Show_ID );
        ...
        // 跳转案件
        let hasAuth = this.hasPageAuth( this.constantPageMap.PAGE_BasicApps_CaseInfoShow_ID );
        if (hasAuth) {
