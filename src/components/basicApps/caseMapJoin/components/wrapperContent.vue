...
import {
  GET_CASE_MAP_JOIN_DATA,
  GET_PERSONS_LIST_BY_CASE_MAP_ITEM
...
export default {
  ...
  methods: {
    ...
    // },
    // 案件/图谱类型筛选
    typeFilter(filterData=this.filterData) {
      const that = this;
      if (that.graphResponseData.nodes == undefined) return;

      let cValue = filterData.caseType; // 选中的 案件value
      let mVale = filterData.mapType; // 选中的 图谱value
      // 筛选条件 为空
      if (!(cValue && cValue.length)) {
        cValue = this.$parent.$parent.$refs.wHeader.typeValue;
      }
      console.log(filterData,'所有数据：',that.graphResponseData);
      // 总数据 为空
      let dataAll = JSON.parse(JSON.stringify(that.graphResponseData));
      const nodes = dataAll.nodes;
      // 筛选 符合的nodes
      var tmpNodes = nodes.filter(data => {
        return cValue.indexOf(data.nodeType) != -1 || mVale.indexOf(data.properties.mapType) != -1
      });
      console.log('符合条件的nodes：',tmpNodes);
      
      // 筛选 不符合的nodes
      let noArr = this.$parent.$parent.$refs.wHeader.typeList.filter(
        x => cValue.indexOf(x.id) == -1
      );
    ...
    keywordFilter(keyword) {
      ...
      let arr = nodes.filter(x => {
        return (
          ...
          (x.properties && hasQuery(x.properties.mapName, keyword)) ||
          (x.properties && hasQuery(x.properties.mapTypeName, keyword))
        );
      });
      ...
      GET_CASE_MAP_JOIN_DATA(params)
        .then(res => {
          ...
          if (res.code == 200) {
            ...
            this.tmpData = data;
            this.$emit("getMapTypeCount",this.tmpData.mapTypeCount);
            this.typeFilter();
            // 加载通知消息
            this._getNotice();
          } else {
    ...
    _getSelectPersonList(params) {
      GET_PERSONS_LIST_BY_CASE_MAP_ITEM(params).then(res => {
        if (res.code == 200) {
