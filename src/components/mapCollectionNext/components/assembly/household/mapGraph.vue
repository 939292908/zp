 ...
 * @LastEditTime : 2019-12-26 18:27:56
...
<template>
  <div class="d3">
    ...
    <map-overview
      ...
      :showDetailBtn="true"
    ></map-overview>
    <!-- 显示详情的弹出框 :mask-style="drawerStyles"
    className="drawer-wrap" v-if="loadModal"
    v-if="loadModal"-->
        ...
        <Tabs v-model="tabIndex" type="card">
          ...
          <template v-if="showPersonInfoTab">
            <TabPane name="t1" :index="2" label="人员详情">
              ...
              <FamilyFormObjectiveView ref="formObjectiveView" :idNumber="idNumber"></FamilyFormObjectiveView>
            </TabPane>
          </template>
        </Tabs>
      </Drawer>
    </template>
    <!-- 编辑工作对象 :z-index="modalIndex"-->
    <template v-if="showWorkObjectiveModal">
      <Modal
...
// import DDrawer from "@/components/assembly/Drawer";
// import FamilyWorkObjective from "./formObjective";
// import FamilyFormObjectiveView from "./formObjectiveView";
// import FamilyFormTpType from "./formTpType";
import {
  getModalMaxIndex,
  createRMenu,
  setMenuData
...
export default {
  ...
  components: {
    // DDrawer,
    // MapOverview: () => import("../mapOverview"),
    MapOverview: () => import("@/components/assembly/MapOverview"),
    // FamilyWorkObjective,
    // FamilyFormObjectiveView,
    // FamilyFormTpType,
    FamilyWorkObjective: () => import("./formObjective"),
    // FamilyFormObjectiveView: () => import("./formObjectiveView"),
    FamilyFormObjectiveView: () => import("../personObjectView"),
    FamilyFormTpType: () => import("./formTpType"),
    RelationModal: () => import("../relationModal")
  },
  data() {
    return {
      loadModal: false,
  ...
  computed: {
    isCanEdit() {
      return this.handleJudgeMenu("FUN_Online_R_Graph_AddEdit_ID");
    },
    isCanEditRelation() {
      return this.handleJudgeMenu("FUN_Online_R_Graph_Edit_Relation_ID");
      // return false;
    },
    isCanDelRelation() {
      return this.handleJudgeMenu("FUN_Online_R_Graph_Remove_Relation_ID");
      // return false;
    },
    modalIndex() {
  ...
  methods: {
    ...
    handleJudgeMenu(role) {
      ...
      if (isEdit == undefined)
        return true && this.hasFunAuth(this.constantFunMap[role]);
      let flag = isEdit && (isEdit == "true" || isEdit == true) ? true : false;
      return flag && this.hasFunAuth(this.constantFunMap[role]);
    },
    // 初始化图形
    initRender() {
      const me = this;
    ...
    buildOpt() {
      ...
      var nodeEditConfig = this.mapTypeList.find(x => x.id == this.mapType)
        .action;
      let option = {
        selector: "#hydra",
        width: width,
        ...
        sideButtonEvt: function(node, sideButton, event) {
          ...
          me.workObjectPersonIdNumber = node.data.properties.SFZHM;
          me.relation = {
            node: node,
            type: sideButton
          };
      ...
      if (nodeEditConfig && this.isCanEdit) {
    ...
    highlightErrCard(idNumbers) {
      ...
      let nodes = this.graph
        .getData()
        .nodes.filter(x => x.data.nodeType == "QB_ZP_RY");
      let allIds = nodes.map(x => x.data.id);
      this.graph.editNodeColor(allIds, "#d7d7d7");
      // 2. 高亮 / 居中
      let errNodes = idNumbers.map(idNumber =>
        nodes.find(x => x.data.properties.SFZHM == idNumber)
      );
    ...
    getGraphData() {
      ...
      apis.GET_ONLINE_MAP_LABEL_BY_ID(obj).then(res => {
        ...
        var data = me.parseData(tmpData, false);
        me.initialize(data);
      });
    },
    // 获取图谱概览的数据
    ...
    handleModalSaveClose() {
      ...
      this.$refs.form
        ...
        .then(res => {
          ...
          this.getGraphData();
          this.getMapOverview();
          // 如果操作的是工作对象，那么刷新图谱的信息
          if (me.personIdNumber == me.coreIdNumber) {
            me.getMapInfo();
          }
          this.showWorkObjective = false;
    ...
    // 编辑关系
    handleRelationEditClick(node) {
      this.$refs.relationModal.handleShowModal();
      this.currentIdNumber = node.properties.SFZHM;
      this.relationName = node.properties.XM;
    },
    // 删除关系
    handleRelationDelClick(node) {
      let me = this;
      this.$Modal.confirm({
        title: "提示",
        render: h => {
          let content =
            node.properties.XM + "（" + node.properties.SFZHM + "）";
          return h("p", [
            "你确定要移除",
            h(
              "span",
        ...
        onOk() {
          apis
            .POST_REMOVE_PERSON_RELATION(node.properties.SFZHM, me.collectId)
            .then(res => {
              if (res.code == 200) {
                this.$Message.success("关系移除成功！");
                me.handleUpdata();
              } else {
                this.$Message.error(res.msg || "关系移除失败！");
              }
            });
        },
        onCancel() {}
