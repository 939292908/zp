 ...
 * @LastEditTime: 2019-11-13 10:32:18
...
<template>
  <Form class="form-panel" ref="form" :model="form" :rules="rule" :label-width="100">
    ...
    <div class="panel-form">
      ...
      <FormItem label="学经起止时间" :class="{validateStar: !isNoIdNumberAdd}">
        <Row>
          <Col span="11">
            <FormItem prop="religionStartTime">
              <DatePicker
                ...
                type="month"
                placeholder="开始时间"
                v-model="form.religionStartTime"
      ...
      <FormItem label="学经经过" prop="religionLearnProcess" :class="{validateStar: !isNoIdNumberAdd}">
        <i-input
          v-model="form.religionLearnProcess"
          type="textarea"
          :autosize="{minRows: 3,maxRows: 6}"
          placeholder="请输入学经经过"
        ></i-input>
...
export default {
  ...
  mounted() {
    this.init();
    this.onStartTimeChange(this.form.religionStartTime);
    this.onEndTimeChange(this.form.religionEndTime);
  },
  methods: {
    init() {
      this.getOptionList();
    ...
    bindFormValue(form) {
      ...
      this.form.religionStartTime = form.religionStartTime
      ...
      this.form.religionEndTime = form.religionEndTime
    ...
    save() {
      ...
      // let req = !isAdd ? apis.PATCH_SAVE_MAP_INFO : apis.POST_MAP_RELATION;
    ...
    fillFormValue() {
      ...
      // 时间格式
      form.religionStartTime = form.religionStartTime ? this.$moment(form.religionStartTime).format("YYYY-MM") : "";
      form.religionEndTime = form.religionEndTime ? this.$moment(form.religionEndTime).format("YYYY-MM") : "";
      return form;
    },
    // 保存提交
    handleSubmit() {
    ...
    getDisplayList() {
      const me = this;
      let mapType =
        this.$parent.curSelectTp || this.$route.query.mapType || "6";
      let arr = [comApis.GET_MAP_RELATIONS(mapType)];
      Promise.all(arr).then(res => {
        this.displayNameAllTypeList = res[0].data;
        ...
        if (this.relation) {
          ...
          this.optionList.displayNameTypeList = arr;
          let isAdds =
            this.$route.query.step == 2
              ? this.$parent.$parent.isAddTwo
              : this.isAdd;
          if (isAdds) {
            if (this.optionList.displayNameTypeList.length > 0) {
              this.relationForm.displayNameCode = this.optionList.displayNameTypeList[0].id;
              this.handleValidateInput();
            }
          } else {
            if (this.relation.node) {
              // 判断一下编辑的节点是不是第一个父节点
              let node = me.relation.node;
              let gxmc = node.data.gxmc;
              if (node.parent && (node.parent.data.nodeType == "QB" || node.parent.data.nodeType == "QB_ZP_TP")) {
                gxmc = "师傅";
              }
              let displayNameObj = this.optionList.displayNameTypeList.find(
              ...
              if (displayNameObj)
                this.relationForm.displayNameCode = displayNameObj.id;
              this.handleValidateInput();
            }
          }
        }
