 ...
 * @LastEditTime: 2019-10-24 17:42:42
        ...
        <Tabs v-model="tabIndex" type="card">
          ...
          <template v-if="showPersonInfoTab">
            <TabPane name="t1" :index="2" label="人员详情">
              ...
              <!-- 人员详情 -->
              <HajjFormObjectiveView
                ref="formObjectiveView"
                :idNumber="idNumber"
                :currentRightMenuNode="currentRightMenuNode"
              ></HajjFormObjectiveView>
            </TabPane>
          </template>
        </Tabs>
...
export default {
  ...
  components: {
    ...
    HajjWorkObjective: () => import("./formObjective"),
    // HajjFormObjectiveView: () => import("./formObjectiveView"),
    HajjFormObjectiveView: () => import("../personObjectView"),
    HajjFormTpType: () => import("./formTpType")
  },
  props: {
    // 图谱结构
    graphType: {
  ...
  data() {
    return {
      hajjType: "", // 朝觐类型 1 公朝   2  私朝
      PeopleStatusActive: "", // 添加时 默认选中
      loadModal: false,
      titleWork: "加载人员信息",
    ...
    NodeLength() {
      if (this.mapFlag == "0") {
        ...
        if (this.tmpData.length) {
          return this.tmpData[1].data.nodes.length;
  ...
  methods: {
    ...
    buildOpt(type) {
      ...
      let option = {
        ...
        sideButtonEvt: function(node, sideButton, event) {
          ...
          // 如果数据第一层有人
          if (me.NodeLength > 0) {
            me.PeopleStatusActive = node.data.type; // 默认选中对应身份
          }
          me.isAddTwo = true;
          ...
          me.relation = {
            node: node,
            type: sideButton,
            relationType: me.PeopleStatusActive
          };
      ...
      // 点击切换图谱，保存图谱结构
      // me.mapFlag =
      //   option.type == "levelTree" ? 0 : option.type == "level" ? 2 : "";
    ...
    getGraphData(mapType) {
      ...
      apis.GET_ONLINE_MAP_LABEL_BY_ID(obj).then(res => {
        // console.log(res);
        // 朝觐类型 1 公朝   2  私朝
        if (me.mapFlag=="2") this.hajjType = res.data[0][0].data.nodes[0].properties.pilgrimageTypeValue;
        // 时间
        ...
        if (res.data.length > 0 && me.mapFlag != "0") {
          ...
          if (me.mapFlag == "2") {
            ...
            let tmpTest = res.data[0].some(
              x => x.id == "1" && x.data.nodes.length > 0
            );
            if (tmpTest) {
    ...
    handleModalSaveClose() {
      ...
      this.$refs.form
        ...
        .catch(error => {
          let errorMsg = !!error ? error.msg || "保存失败" : "请完善表单信息";
          me.saveLoading = false;
    ...
    handlePersonEditClick() {
      ...
      // 判断是否是第一次创建的工作对象
      me.personIdNumber = me.currentRightMenuNode.properties.SFZHM;
      let nodeTmp = me.graph.getNodeByParams(me.currentRightMenuNode.id, "id");
      if (me.isFirstworkObject) {
        me.workObjectPersonIdNumber = "";
        me.relation = {
          node: nodeTmp,
          type: "-1", //如果右键编辑节点，type设置为-1，这身份/称谓字段不能编辑
          relationType: me.currentRightMenuNode.relationType
        };
      } else {
      ...
      // 设置默认选中
      // if(me.$refs.form) {
      //   me.$refs.form.setPeopleStatusActive(me.currentRightMenuNode.relationType); // 传给子组件 默认选中项
      // }
    },
    // 点击图谱概述事件，type：查询位置 item：点击对象
    handleSelectClick(type, item, index) {
      ...
      // console.log(type, item.typeName, item.tagName, nodes);
      console.log(type, item.typeName, item.tagName);
