 ...
 * @LastEditTime: 2019-11-13 10:36:16
...
<template>
  <div class="formTpType-box">
    <Form ref="form" :model="form" :rules="rule" :label-width="labelWidth">
      ...
      <FormItem label="朝觐类别" prop="pilgrimageTypeValue">
        <Select
          v-if="!disable"
          v-model="form.pilgrimageTypeValue"
          :disabled="$route.query.step!=0 || isExcel"
        >
          <Option
            v-for="(item, index) in hajjTypeList"
            :key="index"
            :value="item.value"
          >{{item.displayName}}</Option>
        </Select>
        <span v-else class="input-label">{{form.pilgrimageType}}</span>
      </FormItem>

      <FormItem label="朝觐时间" class="lean">
        <Row>
          <Col span="8">
            <FormItem prop="pilgrimageStartTime">
              <DatePicker
                style="width:90%"
                type="date"
                placeholder="开始时间"
                format="yyyy-MM-dd"
                v-model="form.pilgrimageStartTime"
          ...
          <Col span="2">
            <i class="iconfont icon-jianhao" style="fontSize: 30px;"></i>
          </Col>
          <Col span="8" push="1">
            <FormItem prop="pilgrimageEndTime">
              <DatePicker
                style="width:90%"
                type="date"
                placeholder="结束时间"
                format="yyyy-MM-dd"
                v-model="form.pilgrimageEndTime"
      ...
      <FormItem label="中介时间" class="lean">
        <Row>
          <Col span="8">
            <DatePicker
              style="width:90%"
              ...
              placeholder="开始时间"
              format="yyyy-MM-dd"
              v-model="form.pilgrimageIntermediaryTime"
              :disabled="disable"
              :options="intermediaryTimeOption"
            ></DatePicker>
          </Col>
        </Row>
      </FormItem>
...
export default {
  ...
  data() {
    return {
      startTimeOption: {},
      endTimeOption: {},
      intermediaryTimeOption: {
        //不可选今天之后的日期
        disabledDate(date) {
          return date && date.valueOf() > Date.now();
        }
      },
      hajjTypeList: [], //朝觐类别字典项
      form: {
        mapType: "", //图谱类型
        mapName: "", //图谱名称
        collectId: "", //采集编号
        ...
        pilgrimageType: "", //朝觐 类型（公朝、私朝）
        pilgrimageTypeValue: "", //朝觐 类型value（1、2）
        pilgrimageStartTime: "", //朝觐开始时间
        pilgrimageEndTime: "", //朝觐结束时间
        pilgrimageIntermediaryTime: "" //中介时间
      },
      oldForm: null,
      stateFontColor: {
        提交: "#19be6b",
    ...
    setTreeInput() {
      ...
      this.$store
        .dispatch("getOptionsByKey", {
          key: OPTIONS_LIST.PILGRIMAGE_TYPE,
          apiFun: comApis.GET_OPTIONS_LIST,
          params: OPTIONS_LIST.PILGRIMAGE_TYPE
        })
        .then(res => {
          me.hajjTypeList = res;
          console.log(res);
  ...
  watch: {
    ...
    // 朝觐类型
    "form.pilgrimageTypeValue": {
      handler(val) {
        if (this.hajjTypeList.length > 0 && val) {
          this.form.pilgrimageType = this.hajjTypeList.find(
            item => item.value == val
          ).displayName;
        }
      },
