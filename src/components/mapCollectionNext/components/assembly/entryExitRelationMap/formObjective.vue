 ...
 * @LastEditTime: 2019-10-18 14:16:03
...
<template>
  <Form class="form-panel" ref="form" :model="form" :rules="rule" :label-width="120">
    ...
    <div class="panel-form">
      ...
      <!-- 出境信息 -->
      <div class="panel-title">出境信息</div>
      <FormItem label="出境时间" prop="newPersonOutboundRelationInfo.outboundTime" :class="{validateStar: !isNoIdNumberAdd && isFirst}">
        <Row>
          <Col span="20">
            <DatePicker
              style="width:90%"
              ...
              placeholder="时间"
              format="yyyy-MM-dd"
              v-model="form.newPersonOutboundRelationInfo.outboundTime"
            ></DatePicker>
          </Col>
        </Row>
      </FormItem>
      <FormItem label="是否注销中国国籍" prop="newPersonOutboundRelationInfo.outboundIscancelValue" :class="{validateStar: !isNoIdNumberAdd && isFirst}">
        <Select v-model="form.newPersonOutboundRelationInfo.outboundIscancelValue" filterable>
          <Option
            v-for="(item,index) in optionList.chineseNationalityList"
            :value="item.value"
            :key="index"
          >{{ item.displayName }}</Option>
        </Select>
      </FormItem>
      <FormItem label="出国证件种类" prop="newPersonOutboundRelationInfo.outboundCertificatetypeValue">
        <Select
          v-model="form.newPersonOutboundRelationInfo.outboundCertificatetypeValue"
          filterable
        >
          <Option
            v-for="(item,index) in optionList.certificateList"
            :value="item.value"
            :key="index"
          >{{ item.displayName }}</Option>
        </Select>
      </FormItem>
      <FormItem label="证件号" placeholder="证件号" prop="newPersonOutboundRelationInfo.outboundNumber">
        <i-input v-model="form.newPersonOutboundRelationInfo.outboundNumber"></i-input>
      </FormItem>
    </div>
    <!-- 添加：无证人员 模态框 -->
    <Modal
...
export default {
  name: "EntryExitRelationFormObjective",
  components: {
    TreeFilter,
    NoIdNumber
  },
  ...
  data() {
    ...
    const noIdNumberValidatePass = (rule, value, callback) => {
      // debugger
      if( !this.isFirst && (rule.field == "newPersonOutboundRelationInfo.outboundTime" || rule.field == "newPersonOutboundRelationInfo.outboundIscancelValue")  ) {
        callback();
        }
    ...
    return {
      ...
      form: {
        ...
        currentAddress: {},
        personBaokongInfo: {},
        newPersonOutboundRelationInfo: {
          outboundCertificatetype: "", // 出国证件种类
          outboundCertificatetypeValue: "", // value
          outboundNumber: "", // 证件号
          outboundIscancel: "", // 是否注销中国国籍
          outboundIscancelValue: "", // value
          outboundTime: "" // 出境时间
        }
      },
      hukouAddress: {
      ...
      rule: {
        ...
        "newPersonOutboundRelationInfo.outboundIscancelValue": {
          validator: noIdNumberValidatePass,
          // required: true,
          message: "是否注销中国国籍不能为空",
          trigger: "change"
        },
        "newPersonOutboundRelationInfo.outboundTime": {
          validator: noIdNumberValidatePass,
          // required: true,
          message: '出境时间不能为空',
          trigger: 'blur',
      ...
      optionList: {
        ...
        certificateList: [], //出国证件种类
        chineseNationalityList: [] //是否注销中国国籍
      },
  ...
  methods: {
    ...
    getOptionList() {
      ...
      //出国证件种类
      tool.getCertificateList(this).then(res => { this.optionList.certificateList = res });
      //是否注销中国国籍列表
      tool.getChineseNationalityList(this).then(res => { this.optionList.chineseNationalityList = res });
    },
    // 身份关系清单
    getDisplayList() {
      const me = this;
      let mapType = this.$parent.curSelectTp || this.$route.query.mapType || "3";
    ...
    handleGetPersonInfoByIdNumber() {
      ...
      if (!idNumber) return;
        apis.GET_NEW_PERSON_INFO_BY_ID(idNumber, collectId).then(res => {
          if (res.code == 200) {
            if (res.data.newPersonOutboundRelationInfo == null) res.data.newPersonOutboundRelationInfo = this.form.newPersonOutboundRelationInfo;
    ...
    bindFormValue(form) {
      let data = this.form;
      // 出入境信息
      let resOtb = form.personOutboundRelationInfo;
      let formOtb = data.newPersonOutboundRelationInfo;
      if (resOtb) {
        formOtb.outboundCertificatetype = resOtb.outboundCertificatetype;
        formOtb.outboundCertificatetypeValue =
          resOtb.outboundCertificatetypeValue;
        formOtb.outboundIscancel = resOtb.outboundIscancel;
        formOtb.outboundIscancelValue = resOtb.outboundIscancelValue;
        formOtb.outboundNumber = resOtb.outboundNumber;
        formOtb.outboundTime = resOtb.outboundTime ? this.$moment(resOtb.outboundTime).format("YYYY-MM-DD") : "";
      }
    ...
    save() {
      ...
      // 出入境信息
      let buildOutbound = form.newPersonOutboundRelationInfo;
      buildOutbound.collectId = collectId;
      if (relation) {
        buildOutbound.personIdentity = relation.displayName;
        buildOutbound.personIdentityCode = relation.relationType;
      }

      data.newPersonOutboundRelationInfo = buildOutbound;
      ...
      var promise = new Promise(async (resolve, reject) => {
        ...
        let params = {
          ...
          personOutboundRelationInfo: buildOutbound,
  ...
  watch: {
    // relation: {
    //   handler(val) {
    //     alert(this.optionList.displayNameTypeList.length)
    //     if (val && this.optionList.displayNameTypeList.length > 0) {
    //       let cur = this.optionList.displayNameTypeList.find(item => {
    //         return item.displayName == val.node;
    //       });
    //       if (cur) {
    //         this.relationForm.displayNameCode = cur.id;
    //         if (this.isAdd) {
    ...
    // 是否注销中国国籍
    "form.newPersonOutboundRelationInfo.outboundIscancelValue": {
      handler(val) {
        // 设置对应 displayName
        let item = this.optionList.chineseNationalityList.find(
          item => item.value == val
        );
        if (item)
          this.form.newPersonOutboundRelationInfo.outboundIscancel =
            item.displayName;
      },
      immediate: true
    },
    // 出国证件种类
    "form.newPersonOutboundRelationInfo.outboundCertificatetypeValue": {
      handler(val) {
        // 设置对应 displayName
        let item = this.optionList.certificateList.find(
          item => item.value == val
        );
        if (item)
          this.form.newPersonOutboundRelationInfo.outboundCertificatetype =
            item.displayName;
      },
      immediate: true
    }
