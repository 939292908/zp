 ...
 * @LastEditTime: 2019-11-07 18:05:48
  ...
  <div class="formTpType-box">
    <Form ref="form" :model="form" :rules="rule" :label-width="labelWidth">
      ...
      <FormItem label="案件编号" prop="caseCode">
        <i-input v-if="!disable" v-model="form.caseCode" :autosize="{minRows: 4,maxRows: 6}"></i-input>
        <span v-else class="input-label">{{form.caseCode}}</span>
      </FormItem>

      <FormItem label="案件类别" prop="caseTypeValue">
        <Select v-show="!disable" v-model="form.caseTypeValue" filterable>
          <Option
        ...
        <span v-if="disable" class="input-label">{{form.caseType}}</span>
      </FormItem>

      <FormItem label="三党两集团一组织" class="longLabel" prop="caseBelongThreeOneValue">
        <Select v-show="!disable" v-model="form.caseBelongThreeOneValue" filterable>
          <Option
            v-for="(item,index) in caseBelongThreeOneList"
            :value="item.value"
            :key="index"
          >{{ item.displayName }}</Option>
        </Select>
        <span v-if="disable" class="input-label">{{form.caseBelongThreeOne}}</span>
      </FormItem>

      <FormItem label="案件涉及专案情况" prop="caseProjectInfo">
        <i-input
          v-if="!disable"
          v-model="form.caseProjectInfo"
          type="textarea"
          :autosize="{minRows: 4,maxRows: 6}"
        ></i-input>
        <span v-else class="input-label">{{form.caseProjectInfo}}</span>
      </FormItem>
...
export default {
  ...
  data() {
    return {
      ...
      caseBelongThreeOneList: [], // 三党两集团一组织
      form: {
        mapType: "", //图谱类型
        mapName: "", //图谱名称
        caseCode: "", //案件编号
      ...
      rule: {
        ...
        caseBelongThreeOneValue: [
          {
            required: false,
            message: "三党两集团一组织不能为空",
            trigger: "change"
          }
        ],
        caseProjectInfo: [
  ...
  methods: {
    ...
    getOptionList() {
      ...
      this.$store
        .dispatch("getOptionsByKey", {
          key: OPTIONS_LIST.CASE_BELONG_THREE_ONE,
          apiFun: comApis.GET_OPTIONS_LIST,
          params: OPTIONS_LIST.CASE_BELONG_THREE_ONE
        })
        .then(res => {
          // this.optionList.nationalityList = res;
          this.caseBelongThreeOneList = res;
          console.log(res);
        });
    },
    // 获取登录用户的信息
    ...
    save() {
      ...
      // 再添加两条字段
      data.caseType = me.caseTypeList.find(
        item => item.value == data.caseTypeValue
      ).displayName; //案件类型 label
      data.caseBelongThreeOneValue &&
        (data.caseBelongThreeOne = me.caseBelongThreeOneList.find(
          item => item.value == data.caseBelongThreeOneValue
        ).displayName); //三党两集团一组织 label
