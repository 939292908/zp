 ...
 * @LastEditTime: 2019-11-13 10:34:41
...
<template>
  <Form class="form-panel" ref="form" :model="form" :rules="rule" :label-width="100">
    ...
    <div class="panel-form">
      ...
      <!-- 案件信息 -->
      <div class="panel-title">案件信息</div>
      <FormItem label="涉案人员身份">
        <!-- <span class="input-label">{{casePeopleStatusTranslate[form.peopleStatus]}}</span> -->
        <span class="input-label">{{peopleStatusLabel}}</span>
      </FormItem>
      <!-- v-if="isFirst"  -->
      <FormItem label="处理结果" prop="caseResult" :class="{validateStar: !isNoIdNumberAdd}">
        <Select v-show="!disable" v-model="form.caseResult" placeholder="处理结果" filterable>
          <Option
            v-for="(item,index) in optionList.caseResultList"
            :value="item.value"
            :key="index"
          >{{ item.displayName }}</Option>
        </Select>
        <span v-if="disable" class="input-label">{{form.caseResult}}</span>
      </FormItem>

      <FormItem label="涉及案件身份不明人员情况说明" prop="casePeopleDescribe" class="twoRowLabel">
        <i-input
          v-model="form.casePeopleDescribe"
          type="textarea"
          :autosize="{minRows: 3,maxRows: 6}"
          placeholder="情况说明"
        ></i-input>
...
import NoIdNumber from "../../NoIdNumber"; // 33
export default {
  name: "CaseFormObjective",
  components: {
    TreeFilter,
    NoIdNumber // 44
  },
  ...
  data() {
    ...
    return {
      ...
      peopleStatusLabel: "", // 成员身份的显示label
      noEditor: false, // 不可编辑
      imgSrc: "",
      ...
      form: {
        ...
        caseResult: "", //案件处理结果
        // tempCaseResult: "",
        casePeopleDescribe: "", //涉及案件身份不明人员情况说明
        fiveOnePropertyList: [],
  ...
  computed: {
    ...
    filterPeopleStatusList () {
      return this.isFirst ? this.optionList.peopleStatusList : this.optionList.peopleStatusList.filter(item => item.value != '8');
    },
    displayNameShow() {
      return this.$route.query.step == 4
  ...
  methods: {
    ...
    getDisplayList() {
      ...
      tool.getDisplayList(mapType).then(res => {
        ...
        // 第一次添加时 默认选中第一个
        if (me.NodeLength == 0) {
          me.form.peopleStatus = arr[0].value;
        }
        // 如果没有获取到，就用gxmc从字典项中找
        if (!me.form.peopleStatus) {
          let item = me.optionList.peopleStatusList.filter(
            item => item.displayName == me.gxmc
          )[0];
          me.form.peopleStatus = item ? item.value : me.form.peopleStatus;
        }
      });
    },
    ...
    bindFormValue(form) {
      ...
      // 人员层级 编辑时需要带出来， 添加时不要覆盖默认的
      if (!this.isAdd && !this.gxmc) data.peopleStatus = form.personCaseInfo ? form.personCaseInfo.personIdentityCode + "" : "";
      // 结果
      data.caseResult = form.personCaseInfo ? form.personCaseInfo.personProcessResultValue : "";
      // 说明
      data.casePeopleDescribe = form.personCaseInfo ? form.personCaseInfo.personDescription : "";
      // 
    ...
    save() {
      ...
      // 关系
      relation = {
        displayName: me.optionList.peopleStatusList.filter( item => item.value == me.form.peopleStatus )[0].displayName,
        relationDirection: null,
        relationName: me.optionList.peopleStatusList.filter( item => item.value == me.form.peopleStatus )[0].displayName,
      ...
      // 编辑信息
      let personIdentityItem = me.optionList.peopleStatusList.filter(item => item.value == me.form.peopleStatus)[0];
      let processResultItem =  me.optionList.caseResultList.filter(item => item.dictOrder == me.form.caseResult)[0];
      data.newPersonInfo.personCaseInfo = {
        // 说明
        caseCondition: me.form.casePeopleDescribe,
        // 身份
        personIdentity: personIdentityItem ? personIdentityItem.displayName : "",
        personIdentityCode: me.form.peopleStatus,
        // 结果
        processResult: processResultItem ? processResultItem.displayName : "",
        processResultCode: me.form.caseResult
      };

      let personProcessResultItem = me.optionList.caseResultList.filter(item => item.dictOrder == me.form.caseResult)[0];
      data.newPersonCaseInfo = {
        // 图谱id
        collectId: me.$route.query.collectId,
        // 涉案身份描述
        personDescription: form.casePeopleDescribe,
        // 人员级别
        personIdentityCode: form.peopleStatus,
        personIdentity: personIdentityItem ? personIdentityItem.displayName : "",
        // 结果
        personProcessResultValue: form.caseResult,
        personProcessResult: personProcessResultItem ? personProcessResultItem.displayName : "",
      };
      ...
      var promise = new Promise(async (resolve, reject) => {
        ...
        if (!isAdd) {
          ...
          tmpData.personCaseInfo = data.newPersonCaseInfo;
    ...
    // 供子组件调用设置 peopleStatus
    setPeopleStatusActive(val) {
      this.form.peopleStatus = val;
    }
  },
  watch: {
    "form.peopleStatus": {
      handler(val) {
        ...
        // 表单显示的 人员关系
        this.peopleStatusLabel = this.relationForm.displayName;
      },
      deep: true,
      immediate: false
    },
