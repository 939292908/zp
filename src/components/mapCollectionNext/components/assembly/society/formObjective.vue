 ...
 * @LastEditTime: 2019-11-13 10:35:42
...
<template>
  <Form class="form-panel" ref="form" :model="form" :rules="rule" :label-width="120">
    ...
    <div class="panel-form">
      ...
      <!-- 社会关系情况 -->
      <div class="panel-title" v-if="!displayNameShow">社会关系情况</div>
      <FormItem label="关系/身份" v-if="!displayNameShow">
        <label>{{relationForm.displayName}}</label>
      </FormItem>
      <FormItem label="提供何种帮助" prop="personCaseSocialInfo.helpTypeValue" v-if="!displayNameShow">
        <Select v-model="form.personCaseSocialInfo.helpTypeValue">
          <Option
            v-for="(item,index) in optionList.helpsList"
            :value="item.value"
            :key="index"
          >{{ item.displayName }}</Option>
        </Select>
...
export default {
    ...
    return {
      ...
      form: {
        ...
        personCaseSocialInfo: {
          ...
          // personIdentity: this.relationForm.displayName, //涉案人员身份值
          // personIdentityCode: this.relationForm.displayNameCode //涉案人员身份编号
        }
      },
      hukouAddress: {
        //户籍地址
  ...
  methods: {
    ...
    handleGetPersonInfoByIdNumber() {
      ...
      apis.GET_NEW_PERSON_INFO_BY_ID(idNumber, collectId).then(res => {
        if (res.code == 200) {
          if (res.data.personCaseSocialInfo == null) res.data.personCaseSocialInfo = this.form.personCaseSocialInfo;
    ...
    bindFormValue(form) {
      ...
      let cur = this.optionList.helpsList.find(
        x => x.displayName == form.personCaseSocialInfo.helpType
      );
      form.personCaseSocialInfo.helpTypeValue = cur ? cur.value : "";
      data.personCaseSocialInfo = form.personCaseSocialInfo;
      ...
      let stateObj = this.optionList.personStateList.find(x => {
        return x.value == form.personInfo.stateValue;
      });
      ...
      comApis.GET_PERSON_STATE_SMALL_LIST(stateValue).then(res => {
        ...
        if (me.form.stateDetailValue) {
          if (me.form.personStateList.length > 1) {
            me.form.personStateList[1] = me.form.stateDetailValue;
          } else {
            me.form.personStateList.push(me.form.stateDetailValue);
          }
        }
        this.$nextTick(() => {
    ...
    save() {
      ...
      form.personCaseSocialInfo.collectId = collectId;
      data.newPersonCaseSocialInfo = form.personCaseSocialInfo;
      ...
      var promise = new Promise(async (resolve, reject) => {
        ...
        let params = {
          ...
          personCaseInfo: null,
          personCaseSocialInfo: data.newPersonCaseSocialInfo,
    ...
    fillFormValue() {
      ...
      //提供各种帮助
      let helpsObj = this.optionList.helpsList.find(
        x => x.value == form.personCaseSocialInfo.helpTypeValue
      );
      form.personCaseSocialInfo.helpType = helpsObj ? helpsObj.displayName : "";
      return form;
    },
    // 保存提交
    handleSubmit() {
    ...
    handleReset() {
      ...
      this.form.personCaseSocialInfo = {};
      this.relationForm = {
        displayName: "", //身份/称谓
        displayNameCode: "", //身份/称谓
        relationName: "工作对象", //关系名称
    ...
    handleDisplayNameChange(cur) {
      ...
      this.relationForm.relationName = displayNameObj
        ? displayNameObj.name
        : "";
      this.relationForm.displayName = displayNameObj
        ? displayNameObj.displayName
        : "";
      this.form.personCaseSocialInfo.personIdentity = this.relationForm.displayName;
      this.form.personCaseSocialInfo.personIdentityCode = this.relationForm.displayNameCode;
      if (cur) {
        this.showDisplayNameError = false;
      }
    },
    ...
    getOptionList() {
      ...
      this.$store
        .dispatch("getOptionsByKey", {
          key: OPTIONS_LIST.HELP_TYPE,
          apiFun: comApis.GET_OPTIONS_LIST,
          params: OPTIONS_LIST.HELP_TYPE
        })
        .then(res => {
          this.optionList.helpsList = res;
        });
    },
    // 身份关系清单
    getDisplayList() {
      const me = this;
      let mapType =
        this.$parent.curSelectTp || this.$route.query.mapType || "5";
      let arr = [comApis.GET_MAP_RELATIONS(mapType)];
      Promise.all(arr).then(res => {
        this.optionList.displayNameTypeList = res[0].data;
        this.getRelationCodeByDisplayName(this.relation);
      });
    },
    // 根据父级传过来的relation，带出身份称谓
    getRelationCodeByDisplayName(relation) {
      if (relation && this.optionList.displayNameTypeList.length > 0) {
          let cur = this.optionList.displayNameTypeList.find(item => {
            return item.displayName == relation.node;
          });
          if (cur) {
            this.relationForm.displayNameCode = cur.id;
            this.relationForm.displayName = cur.displayName;
            if (this.isAdd) {
  ...
  watch: {
    relation: {
      handler(val) {
        this.getRelationCodeByDisplayName(val);
      },
      deep: true,
      immediate: true
    },
