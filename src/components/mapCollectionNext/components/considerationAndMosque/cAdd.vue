<template>
  <div class="info">
    ...
    >
      <div class="panel-form">
        <FormItem label="职位" prop="positionValue">
          <Select v-model="partyBranch.positionValue" filterable placement="top">
            <Option
              v-for="(item,index) in optionList.positionPartyList"
              :value="item.value"
              :key="index"
            >{{ item.displayName }}</Option>
          </Select>
      ...
      <div class="panel-form">
        <FormItem label="分管工作" prop="work">
          <i-input v-model.trim="partyBranch.work"></i-input>
        </FormItem>
          ...
          <Row>
            ...
            <Col span="6">
              <Checkbox
                ...
                :disabled="partyBranch.departureYear!=null||partyBranch.departureMouth!=null"
    ...
    <div class="btnGrounp">
      <Button type="primary" @click="saveAndClose(1)">保存并关闭</Button>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    fillFormValue() {
      ...
      // 职位
      let positionObj = this.optionList.positionPartyList.find(
          ...
          that.$refs["partyBranchInfo"].validate(val => {
            if (val) {
              ...
              apis.savaParty(form).then(res => {
                if (res.code == 200) {
                  that.$Message.success("村两委人员信息保存成功!");
                  console.log(res);
              ...
              that.$Message.warning("请完善村两委任职信息！");
            }
          });
        } else {
