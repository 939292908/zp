 ...
 * @LastEditTime: 2019-11-13 10:35:53
  ...
  >
    <div class="formLeft">
      ...
      <FormItem label="姓名" prop="name">
        <i-input v-model="form.name" :disabled="disabledInput"></i-input>
      </FormItem>
      ...
      <Row>
        <Col span="12">
          <FormItem label="性别" prop="genderValue" :class="{validateStar: !isNoIdNumberAdd}">
            <Select v-model="form.genderValue" :disabled="!isNoIdNumberAdd || disabledInput">
              <Option
                :key="index"
        ...
        <Col span="12">
          <FormItem label="民族" prop="nationalityValue" :class="{validateStar: !isNoIdNumberAdd}">
            <Select v-model="form.nationalityValue" :disabled="disabledInput" filterable>
              <Option
                :key="index"
      ...
      <FormItem label="政治面貌" prop="politicsStatusValue">
        <Select v-model="form.politicsStatusValue" :disabled="disabledInput">
          <Option
    ...
    <div class="formRight">
      <FormItem label="职业" prop="professionValue">
        <Select v-model="form.professionValue" filterable :disabled="disabledInput">
          <Option
      ...
      <FormItem prop="currentdetailAddress">
        <i-input
          ...
          placeholder="请输入现住址详细地址"
        ></i-input>
      </FormItem>
      <FormItem label="服务场所" prop="servicePlace">
        <i-input v-model="form.servicePlace" :disabled="disabledInput"></i-input>
      </FormItem>
      <div class="panel-title">社会信息</div>
      <FormItem label="银行卡" prop="socialBankCard">
        <i-input v-model="form.socialBankCard" :disabled="disabledInput"></i-input>
      </FormItem>
      <FormItem label="车辆" prop="socialCar">
        <i-input v-model="form.socialCar" :disabled="disabledInput"></i-input>
      </FormItem>
      <FormItem label="电话" prop="socialPhone">
        <i-input v-model="form.socialPhone" :disabled="disabledInput"></i-input>
      </FormItem>
      <FormItem label="QQ号码" prop="socialQq">
        <i-input v-model="form.socialQq" :disabled="disabledInput"></i-input>
      </FormItem>
      <FormItem label="微信号码" prop="socialWechat">
        <i-input v-model="form.socialWechat" :disabled="disabledInput"></i-input>
      </FormItem>
    </div>
    <!-- 添加：无证人员 模态框 -->
    <Modal
...
export default {
  ...
  data() {
    ...
    return {
      ...
      taskId: "", // GET_NEW_PERSON_INFO_BY_ID
      newCollectId: "",
      changeOnSelect: false,
      showIdNumberError: false,
      disabledInput: false, //输入框禁止
      newDisabledInput: false, //编辑下身份证输入框禁止
      imgSrc: "",
  ...
  watch: {
    ...
    idNumber: {
      handler(val) {
        // 编辑人员
        if (!val || this.$parent.$parent.isAdd == null) {
          return;
        }
        let collectId = this.collectId || this.$route.query.collectId;
        apis
          .GET_NEW_PERSON_INFO_BY_ID(val, collectId)
          .then(res => {
            if (res.code == 200) {
              // console.log(res);
              ...
              this.$parent.$parent.name = res.data.personInfo.name;
            }
          })
          .catch(respanse => {});
  ...
  methods: {
    ...
    getNoIdNumberDataByAdd(data) {
      ...
      this.isShowNoIdNumberModal = false;
      this.handleGetPersonInfoByIdNumber(this.form.idNumber);
    },
    // 添加无证人员的 关闭按钮事件
    cancelClickToAdd() {
      this.isShowNoIdNumberModal = false;
    ...
    handleGetPersonInfoByIdNumber() {
      ...
      if (idNumber) {
        ...
        this.$parent.$parent.getOutboundinfoList(idNumber);
        apis.GET_NEW_PERSON_INFO_BY_ID(idNumber, collectId).then(res => {
          // this.handleReset();
          if (res.code == 200 && res.data != null) {
            this.bindFormValue(res.data.personInfo);
            this.$parent.$parent.name = res.data.name;
            this.$nextTick(() => {
    ...
    save() {
      ...
      console.log(form);
      this.$parent.$parent.name = form.name;
      let params = {
    ...
    delete() {
      ...
      var promise = new Promise((resolve, reject) => {
        apis.deleteEenterPersonnel(that.idNumber, params).then(res => {
          if (res.code == 200) {
            resolve(res);
          } else {
            reject(res);
