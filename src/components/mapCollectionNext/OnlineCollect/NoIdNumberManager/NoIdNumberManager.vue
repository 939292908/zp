  ...
  <div class="right-content-box">
    ...
    <div class="noIdNumberManager-content">
      ...
      <div class="action-list">
        ...
        <p class="search-result">本次搜索耗时{{titleDetail.executeTime}}秒，搜索出{{titleDetail.total}}条包含关键字信息的图谱</p>
        <Tabs
          type="card"
          :animated="false"
          :value="filters.state | format"
          @on-click="tabChange"
        >
  ...
  methods: {
    ...
    getCurTableData() {
      ...
      let params = {
        ...
        needCollect: this.filters.isNeedVerify.length==1 ? this.filters.isNeedVerify[0] : "",
      ...
      req(params).then(res => {
        if (res.code === 200) {
          ...
          // 加序号
          data.forEach((val, index) => val.mapNumber = index + 1 + (this.pagination.pageIndex - 1) * 10);
          // table 数据
          this.tableData = data;
          // 分页
    ...
    setColumns() {
      this.columns1 = [
        ...
        },
        {
          ...
          render: (h, params) => {
            let data = params.row;
            return <div>{data.stateStrong+(data.stateDetail ? ' / '+data.stateDetail : '')}</div>;
          },
        },
        {
          title: "操作",
          ...
          render: (h, params) => {
            return (
              <span>
                {this.hasFunAuth( this.constantFunMap.FUN_Online_MapReivew_UnSubmit_Edit_ID ) || true ? (
                  <i-button
                    size="small"
                    type="primary"
                    on-click={$event =>
      ...
      this.columns2 = [
        ...
        },
        {
          ...
          render: (h, params) => {
            return (
              <span>
                {this.hasFunAuth( this.constantFunMap.FUN_Online_MapReivew_Review_History_ID ) || true ? (
                  <i-button
                    size="small"
                    type="primary"
