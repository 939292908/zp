...
<template>
  <div class="bigMarkerMapArea">
    ...
    <div class="markerContent">
      <div class="markerContentLeft">
        <div
          class="mapContent"
          id="allMap"
          @mousedown="mousedownClear()"
          @mouseover="mouseoverChangeCursor($event)"
        ></div>
        <div class="mapTitle village">{{villageName}}</div>
        <div class="operation" v-if="$route.query.isAudit==null">
          <Button
            type="primary"
            @click="submission()"
            v-if="hasSubmit && hasFunAuth(constantFunMap.FUN_Online_F_Map_Enter_Submit_ID) && mapInfo.createId == userId"
          >提交</Button>
          <Button
            type="primary"
    ...
    <!-- 出入境人员处理信息 -->
    <Modal v-model="modalShow" class="exitEnter" footer-hide fullscreen :closable="false">
      <span class="closeBtn" @click="cancelAdd()">
        <i class="iconfont icon-guanbi1"></i>
      </span>
      <div class="left">
        <div class="item">
          <div class="stepOne" @click="stepIndexChange('#info1',1)" :class="{'act':stepIndex == 1}">
            <a id="info11">人员基本信息</a>
          </div>
          <div
            class="stepTwo"
            v-if="enterRecordShow"
            @click="stepIndexChange('#info2',2)"
            :class="{'act':stepIndex == 2}"
          >
            <a id="info12">出入境记录</a>
          </div>
        </div>
      </div>
      <div class="right" id="right" ref="modal">
        <div class="topAddPersonnelInfo" id="info1">
          <div class="header">
            <div class="title">
              {{title}}
          ...
          <addPersonnelModal
            ...
            :villageName="villageName"
            :isAdd="isAdd"
            :villageMapID="villageMapID"
            :idNumber="idNumber"
            :listsDone="listsDone"
            :listsUnfinished="listsUnfinished"
          ></addPersonnelModal>
          <div class="foot">
            <div class="havaStepTwo" v-if="havaStepTwo">
              <Button
                type="info"
                v-show="enterRecordShow && hasFunAuth(constantFunMap.FUN_Online_F_Map_Enter_Add_ID)"
              ...
              <Button
                ...
                @click.native="handleModalSaveClose"
              >保存并下一步</Button>
              <Button type="info" @click="cancel()" v-if="!enterRecordShow">取消</Button>
            </div>
            <div class="noneStepTwo" v-else>
              <Button type="info" @click="editPersonnelInfo()">编辑</Button>
              <!--  -->
            </div>
          </div>
        </div>
        <div class="bottomenterInfo" id="info2" v-if="enterRecordShow">
          <div class="header">
            <div class="infoTitle">
              <div class="titleLeft">出入境记录</div>
              <div class="titleRight" @click="editAddRecord()">
                添加
        ...
        <div class="exit" v-if="!havaStepTwo">
          <Button
            type="info"
            v-if="!isAdd && hasFunAuth(constantFunMap.FUN_Online_F_Map_Enter_Delete_ID)"
            @click.native="handleModalDeleteClose"
          >删除人员</Button>
          <Button type="info" @click="cancelAdd()">关闭</Button>
        </div>
      </div>
    </Modal>
    <!-- 人员详情 -->
    ...
    >
      ...
      <div class="color">
        <div class="left">
          <span>危害等级</span>
          <i :style="'background:#'+donePersonnelInfo.personOutboundRecordInfo.harmColor"></i>
        </div>
        <div class="right">
          <span>风险等级</span>
          <i :style="'background:#'+donePersonnelInfo.personOutboundRecordInfo.riskColor"></i>
        </div>
      </div>
    ...
    <Modal v-model="errorModal" width="580" class="errorBox">
    ...
    <div class="error-box" v-if="hasSubmit&&errorInfo.errorData.length>0">
...
import operationHeader from "./components/profile/operationCollect";
import detailsCollect from "./components/profile/personnelDetailsCollect"; //right
import personnelList from "./components/profile/personnelList"; //right
import addPersonnelModal from "./components/profile/formAddPersonnelEnter"; //添加或编辑人员信息
import enterRecord from "./components/profile/enterRecord"; //出入境记录
...
import personnelEntryExitInfo from "./components/assembly/entryExitInfo/personnelEntryExitInfo";
import personInfoDetail from "./components/profile/personInfoDetail/personInfoDetail"; //查看详情
...
export default {
  ...
  methods: {
    ...
    mapForOver(e, data, key) {
      ...
      // "left:" + e.x + "px;top:" + e.y + "px;max-height:" + maxHeight + "px";
      /* var scrollHeight = document.body.scrollHeight - 98; //头部，底部共98
      var scrollWidth = document.body.scrollWidth; //340是人员列表的宽度
      var maxHeight = scrollHeight - e.x;
    ...
    isvillageArea(e) {
      ...
      if (that.$refs.list != undefined) {
        that.$refs.list.indexStyle = null;
      }
      /* var result = this.teamPolygonArr.some(function(team) {
        return BMapLib.GeoUtils.isPointInPolygon(e.point, team);
      }); */
      var markerI = that.markerI;
      var markerIndex = that.markerIndex;
      if (markerIndex != null && markerI == 2) {
        if (e.target.markerObj != null) {
          that.moveMarker(e, 2);
          that.changePoint(e.target.markerObj, e.point);
        } else {
          var obj = that.listsUnfinished[markerIndex];
          that.changePoint(obj, e.point);
          that.moveMarker(e, 1);
        }
      } else if (markerIndex != null && markerI == 1) {
        that.markerDraagend(that.markerListArr[markerIndex], e.point);
      }
    ...
    getPersonnelListDetail(key, i) {
      ...
      that.$refs["personInfoDetail"].tabIndex = "t1"; //人员详情
      that.$refs["personInfoDetail"].showDetail = true; //人员详情
      that.$refs["personInfoDetail"].showDetailDrawer = true; //人员详情
      that.$refs["personInfoDetail"].showPersonInfoTab = true; //人员详情
    },
    // 添加人员/人员列表
    getFun(data) {
      if (data == "add") {
    ...
    markerDraagend(e, point = null) {
      ...
      if (e.target == undefined) {
        e.point = point;
        this.map.removeOverlay(this.labelListArr[e.index]);
        this.map.removeOverlay(this.markerListArr[e.index]);
        this.moveMarker(e, 2, e.index);
        this.changePoint(e.markerObj, point);
      } else {
    ...
    moveMarker(e, i = 1, index) {
      ...
      if (that.hasSubmit) {
        if (
          that.hasFunAuth(that.constantFunMap.FUN_Online_F_Map_Enter_Delete_ID)
        ) {
          markerMenu.addItem(
            new BMap.MenuItem("取消打点", that.removeMarker.bind(marker))
          ); //添加右键移除打点人员
        }
        // 3、编辑
        if (
          ...
          markerMenu.addItem(
            new BMap.MenuItem("编辑信息", that.editPersonDetails.bind(marker))
          ); //添加右键查看详细信息newMarkerObjDetail  editDetailInfo
        }
      }
    ...
    personnelSketch(e) {
      ...
      var perhapsHeight = "";
      // 判断
      perhapsWidth = e.pixel.x + frameWidth > scrollWidth ? "left" : "right";
      perhapsHeight =
        e.pixel.y + frameHeight > scrollHeight && e.pixel.y > frameHeight
          ? "top"
          : "bottom";
      if (perhapsWidth == "left" && e.pixel.x + frameWidth > scrollWidth) {
        this.sketchLeft = scrollWidth - frameWidth + 15;
      } else if (perhapsWidth == "right" && e.pixel.x < frameWidth) {
        this.sketchLeft = e.pixel.x + 15;
      } else {
        this.sketchLeft = e.pixel.x + 15;
      }
      if (perhapsHeight == "bottom" && e.pixel.y < frameHeight) {
        this.sketchTop = e.pixel.y + 35;
      } else if (
        perhapsHeight == "top" &&
        e.pixel.y + frameHeight > scrollHeight
      ) {
        this.sketchTop = e.pixel.y - frameHeight;
      } else {
        this.sketchTop = e.pixel.y;
      }
      console.log(perhapsWidth, perhapsHeight); */
      ...
      this.$nextTick(()=>{
        console.log(this.$refs.sketch.offsetHeight);
      })  
    },
    // 人员详情personnelDetail
    ...
    // 编辑
    editPersonDetails(e, ee, marker) {
      const that = this;
      that.editDetailInfo(marker.markerObj);
    },
    // get组队中心点
    borderCenter(list) {
      var lngAdd = 0,
    ...
    handleShowDetail() {
      ...
      this.isMap = "map"; */
      const that = this;
      that.$refs["personInfoDetail"].tabIndex = "t0"; //人员详情
      that.$refs["personInfoDetail"].showDetail = true; //人员详情
      that.$refs["personInfoDetail"].showDetailDrawer = true; //人员详情
      that.$refs["personInfoDetail"].showPersonInfoTab = false; //人员详情
    },
    // 取消
    cancel() {
      ...
      that
        ...
        .then(res => {
          if (res == 1) {
            ...
            that.considerationAndMosqueMarker();
          }
        })
        .catch(res => {
          console.log(res);
    ...
    isSubmission() {
      ...
      apis.POST_CHANGE_MAP_STATE(params).then(res => {
        if (res.code === 200) {
          ...
          that.errorInfo.message = res.data.message;
          that.errorInfo.errorData = [
    ...
    mouseoverChangeCursor(e) {
      ...
      if (that.markerI == 1) {
        color = mapLists.color(
          this.listsDone[this.markerIndex].personOutboundRecordInfo.harmLevel ||
            "红"
        );
      } else {
        color = mapLists.color(
    ...
    getOutboundinfoList(id = null) {
      ...
      var collectId = { collectId: this.$route.query.collectId };
      apis
