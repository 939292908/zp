...
<template>
  <div class="bigMarkerMapArea">
      ...
      <div class="markerContentLeft">
        ...
        <div class="operation" v-if="$route.query.isAudit==null">
          <Button
            type="primary"
            @click="submission()"
            v-if="hasSubmit && hasFunAuth(constantFunMap.FUN_Online_F_Map_Profile_Submit_ID)&&  mapInfo.createId == userId"
          >提交</Button>
          <Button
            type="primary"
    ...
    <!-- home -->
    <div class="home" v-show="cursorHome"></div>
    <!-- loading -->
...
import addPersonnelModal from "./components/profile/formAddPersonnel"; //添加或编辑人员信息
...
import addBeHitPerson from "./components/profile/addPeopleBeHit"; //添加人员被打击信息
...
export default {
  ...
  methods: {
    ...
    init() {
      ...
      me.considerationAndMosqueMarker(); // 打点村党委清真寺
      me.screenString = "";
      me.screenIndex = null;
    },
    ...
    rightAddPersonnel(e, ee, polygon) {
      this.coordinate = e;
      this.$refs["addBeHitPerson"].showModal = true;
      // this.$refs.addBeHitPerson.init();
      this.$refs.addBeHitPerson.longitude = e.lng;
      this.$refs.addBeHitPerson.latitude = e.lat;
    },
    // 判断是否在存范围之内
    isvillageArea(e) {
      const that = this;
      ...
      if (markerIndex != null && markerI == 2) {
        ...
        console.log(1);
        that.markerDraagend(this.markerListArr[markerIndex], e.point);
        /* var obj = this.listsDone[markerIndex];
        // console.log(obj)
    ...
    // 添加人员/人员列表
    getFun(data) {
      this.$refs["addBeHitPerson"].showModal = false;
      if (data == "add") {
        this.idNumber = "";
        this.$refs["addBeHitPerson"].showModal = true;
        this.isAdd = true;
    ...
    moveMarker(e, i = 1, index) {
      ...
      if (that.hasSubmit) {
        if (
          that.hasFunAuth(
            that.constantFunMap.FUN_Online_F_Map_Profile_Delete_ID
          )
        ) {
          markerMenu.addItem(
    ...
    editDetailInfo(data) {
      ...
      this.$refs["addBeHitPerson"].showBeHit = true;
      this.$nextTick(() => {
        this.isAdd = false;
        this.$refs["addBeHitPerson"].$refs["baseBeHit"].caseInfo(this.idNumber);
      });

      this.$refs["addBeHitPerson"].$refs[
        "right-form"
      ].form.idNumber = this.idNumber;
      this.$refs["addBeHitPerson"].$refs[
        "right-form"
    ...
    cancel() {
      ...
      this.$refs.addBeHitPerson.handleReset();
      this.idNumber = "";
      this.$refs["addBeHitPerson"].showModal = true;
    ...
    handleModalSaveClose() {
      ...
      const me = this;
      me.saveLoading = true;
      this.$refs.addBeHitPerson
        .handleSubmit()
        .then(res => {
          me.$refs.addBeHitPerson.handleReset();
          me.saveLoading = false;
          me.$Message.success("保存成功");
          me.initLoad(2);
          me.rightContent = "list";
          me.$refs["addBeHitPerson"].showModal = false;
        })
        .catch(error => {
          let errorMsg =
    ...
    handleModalDeleteClose() {
      this.$refs.modal.scrollTop = 0;
      const me = this;
      me.$refs.addBeHitPerson.handleSubmitDelete().then(res => {
        if (res.code == 200) {
          me.$refs.addBeHitPerson.handleReset();
    ...
    mouseoverChangeCursor(e) {
      ...
      if (that.markerI == 1) {
        color = mapLists.color(
          that.listsDone[that.markerIndex].personProcessRecordInfo
            .caseTypeColorName || "红"
        );
      } else {
        color = mapLists.color(
          that.listsUnfinished[that.markerIndex].personProcessRecordInfo
            .caseTypeColorName || "红"
        );
      }
      /* var color = mapLists.color(
        that.listsUnfinished[ththatis.markerIndex].personProcessRecordInfo
          .caseTypeColorName
      ); */
      var allMap = document.getElementById("allMap");
