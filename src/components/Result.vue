...
<template>
  <div class="main_cont" @click="handleWrapperClick($event)">
          ...
          <div class="cont">
            <div class='div_cont'>
              <div class="header_p" >
                ...
                <span class="check_res" v-on:click ="checkArchive(index)" @click.stop="statusList.PersonInforShow =true">查看档案</span>
                <div class="bg-list">
              ...
              <div class="div_imgcont">
                <img class="img_width" v-lazy="item.property.imagePath">
                ...
                <div class="h_pdiv">
                  ...
                  <div class="h_p">
                    ...
                    <span class='span_cor' :title="item.property.profession?item.property.profession:''">{{item.property.profession?item.property.profession:'无'}}</span>
                  </div>
                  <div class="h_p">
                    <span class='span_cor2'>手机号</span>
                    <span class='span_cor' :title="item.property.phone?item.property.phone:''">{{item.property.phone?item.property.phone:'无'}}</span>
                  </div>
                  <div class="h_p">
                    <span class='span_cor2'>政治面貌</span>
                    <span class='span_cor' :title="item.property.politicsStatus?item.property.politicsStatus:''">{{item.property.politicsStatus?item.property.politicsStatus:'无'}}</span>
                  </div>
                </div>
              </div>
              <div style="clear: both">
                ...
                <div v-if="activeIndex==index" class="tp-list-box">
                  <div v-for = "(items,index) in item.tpData" :key ='index' class="list-box scrollbar">
                      <div class="p_cont">
                        <span class="span_title" @click="toArchivesJoin(items.tpid,item.rid,item.property.SFZHM)">{{items.tpms}}</span>
                        <div class="icon-list">
                          <span class="span_img1" v-if="hasFunAuth(constantFunMap.FUN_Search_Result_CHUANBING_ID)" @click="join(items.tpid,item.rid,items.mapCode,item.property.SFZHM)">
                            <img src="../../static/images/result_img/r4.png" title="图谱串并">
                          </span>
                          <span class="span_img1" v-if="hasFunAuth(constantFunMap.FUN_Search_Result_DIEJIA_ID)" @click="overlaptp(items,item)">
                            <img src="../../static/images/result_img/r5.png" title="图谱叠加">
                          </span>
                          <span class="span_img1" @click="toArchivesJoin(items.tpid,item.rid,item.property.SFZHM)">
    ...
    <transition name="personInforArea">
      <!-- <PersonInfor ref="PersonInfor" :node="curEneityData" :modal="!curEneityAllData.eneityTrajectoryInfo.dataTypes ? '' : curEneityAllData" v-if="statusList.PersonInforShow" @listenToChild="hidePersonInfor" @switchTrajectory="switchTrajectory" @changeTime="changeTime" class="personInfo"></PersonInfor> -->
      <ArchivalInfor v-show="statusList.PersonInforShow" @listenToChild="hidePersonInfor" :tpInfo="tpInfo"></ArchivalInfor>
    </transition>
    <!-- </div> -->
...
import { transferParameters } from "@/tools/units.js";
    ...
    methods:{
      ...
      toArchivesJoin (tpid,rid,sfzhm) {
        let archivesjoin = this.$router.resolve({
          path: '/archivesjoin',
          query: {
            tpids : tpid,
            rid : rid,
        ...
        window.open(archivesjoin.href, '_blank');
      },
      join (tpid,rid,tpCode,sfzhm) {
        let params = tpCode
        let path = '/join'
        let data = {
          tpid : tpid,
        ...
        transferParameters(this,path,params,data,'areaMapCodes')
      },
      overlaptp (items,item) {
        // console.log(items);
        // console.log(item);
        this.$router.push({
          path: '/overlaptp',
      ...
      checkArchive(index) {
          ...
          // this.curEneityAllData.imgSrc = this.getImagePath(this.datalist[index].property.picUrl);
          this.tpInfo=this.datalist[index]
          this.tpInfo.properties=this.datalist[index].property
        ...
        .then(response => {
          ...
          if (response.code == 200) {
            ...
            this.datalist.forEach((item)=>{
              ...
              : !item.property.profession ? item.property.profession = ""
              : !item.property.politicsStatus ? item.property.politicsStatus = "" : "?";
              item.property.imagePath = this.getImagePath(item.property.picUrl);
            })
      ...
      getCorrelationInfo () {
        ...
        params.append('token', me.token);
        //params.append('nodeID', me.rid_id);
        params.append('type', me.node_type);
