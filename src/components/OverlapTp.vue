<!--
 * @Author: your name
 * @Date: 2019-10-14 18:01:25
 * @LastEditTime: 2019-10-28 18:14:58
 * @LastEditors: your name
 * @Description: In User Settings Edit
 * @FilePath: \map-web\src\components\OverlapTp.vue
 -->
<template>
  <div id="overlapTp" class="result" @click="myNavModalShow=false;statusList.uploadImg=false;statusList.personModalShow=false;statusList.newsModalShow=false;statusList.appModalShow=false;statusList.messageDetailsShow=false;statusList.guidelinesShow=false;statusList.feedbackShow=false;">
    <header>
      <div class="logo">
    ...
    <!-- 个人 -->
    <personModal v-show="statusList.personModalShow" ref="personModal" @openMyNavModal="openMyNav" @openFeedbackModal="openFeedback" @openGuideModal="openGuide"></personModal>
    <!-- 消息 -->
    ...
    <div class="result-cont">
      ...
      <div id="d3" class="d3">
        ...
        <div class="graphical" v-show="statusList.d3Show">
          <!-- 风险要素组件 -->
          <riskLevel :assessData="assessData" ref="risk" @highlightNodes="highlightNodes"></riskLevel>
    ...
    <transition name="multiStr">
      <div class="multi-str" v-show="statusList.MultiModal">
        <div class="condition-modal">
          ...
          <div class="cont-box">
            <div class="cont-list" v-for="(items,index) in multiStrData" :key="index+'multi'" :data-index="index">
              <ul class="condition-select overlap-tp">
                <li class="li tp-unit">
                  <span>地区单位：</span>
                  <!-- <input type="text"> -->
              ...
              <div class="tp-icon">
                ...
                <span class="sure" v-if="hasFunAuth(constantFunMap.FUN_OverlapTp_Showpic_ID)" @click="getTreeData(true)">展示图形</span>
              </div>
            </div>
            <div class="cont-list" v-for="(items,index) in overlapData" :key="index" :data-index="index">
              <ul class="condition-select">
                <li class="li family-description">
                  <span class="overlap-tp">可叠加图谱名称：</span>
                  <div class="wrapper">
                    <div class="box">
                      <!-- 多选 (后期备用)-->
                      <!-- <div style="border-bottom: 1px solid #e9e9e9;padding-bottom:6px;margin-bottom:6px;padding-left:10px;">
                    ...
                    <div class="loading-tpmc" v-show="statusList.loadingoverLapTpmcShow">
                      <div class="loader-tpmc"></div>
                    </div>
                  </div>
                </li>
                <li class="page-tpmc">
                  <Page :total="items.pageTotal" :page-size="10" :current="items.pageIndex" @on-change="getPageTpIndex"/>
                </li>
              </ul>
            </div>
          </div>
          ...
          <div class="cont-btn">
            <span class="cancel" @click="cancelOverlap">取消叠加</span>
            <span class="sure" v-if="hasFunAuth(constantFunMap.FUN_OverlapTp_StartAdd_ID)" @click="getOverlapAfterData">开始叠加</span>
          </div>
    ...
    <!-- 图谱叠加按钮 -->
    <div class="dtcb" @click="ShowMulti">
      <span>图谱叠加</span>
    </div>
    <!-- 历史记录 -->
            ...
            <div class="infor-details bar">
              ...
              <Timeline>
                <TimelineItem v-for="(item,index) in historyData" :key="index">
                  ...
                  <div class="icon" :style="{background:item.color}" @click="againGetTree(item.id,item.oid)">
                    <span>{{ item.type }}</span>
                  </div>
...
export default {
  ...
  data () {
    return {
      ...
      keyword: '',
      language: '',
      tpidOverlap: this.$route.query.tpid,  //智能搜索页面图谱跳转过来判断（备用）
  ...
  created () {
    ...
    if(!this.rid.length){
      console.log(this.rid,484484);
      this.rid.push(this.$route.query.rid); //首次进来高亮绿色的那个id
    }
    if(this.$route.query.tpid != undefined){
      this.multiStrData[0].AllGroup.push(this.$route.query);
      this.multiStrData[0].radio = this.$route.query.tpid;
    };

    //if(!sessionStorage.achiveJoinHighlightId){
      sessionStorage.overlapTpHighlightId = this.$route.query.rid || '';
    //}
    ...
    //this.multiStrData[0].filterCondition = eval("("+this.$store.getters.citys+")");
  },
  mounted(){
    const me = this;
    ...
    if(this.tpidOverlap){  //智能搜索页面图谱跳转过来判断（备用）
  ...
  methods : {
    ...
    getEntityLinksConfig () {
      ...
      this.$axios.get(url, params).then(response => {
        ...
        me.multiStrData[0].tplxList = response.data.tplxMapping
    ...
    buildOpt(){
      ...
      return {
        ...
        highlightText : '',
        showToptagMenu : true,  //实体顶部标签鼠标移上，显示图谱列表
        ...
        topTagClickEvt : function(data){
          ...
          //   me.historyData = [];
          // }
          me.multiStrData[0].radio = data.tpid;
          me.historyData.unshift(me.addHistory);
          me.rid = [];
          me.rid.push(data.oid);  //从哪个进入图谱，绿色高亮
    ...
    getJzmc (flag) {
      ...
      let url = me.joinPath + 'MapSuperpositionResource/v1/SuperpositionMaps/keyword';
      ...
      .then(response => {
        ...
        me.multiStrData[0].pageTotal = response.data.total;
      })
      .catch(error => {
     ...
     * @author erbing
     */
    highlightNodes (label,color){
      var me = this;
      me.resultLabel = label;

      me.resultNodes = [];
      me.resultId = [];
      //1 再次点击选取清空上一次选择的内容，并且清空高亮的ids数组
      if (me.highlightNodeIDs.length!=0) {
        me.highlightNodeIDs.forEach(function(item){
          me.graph.showKeyNode(true,item.oid,'#d7d7d7');
        });
        me.highlightNodeIDs=[];
      }
      //2 找到这次点击标签相关的ID，放到me.highlightNodeIDs里面
      me.filterHighlightIds(me.datas.children,me.resultNodes);
      //3 根据找到的ID进行高亮

      me.highlightNodeIDs.forEach(function(item){
        me.graph.showKeyNode(true,item.oid,'blue');
        let node = me.graph.getNodeByParams(item.id,'id')
        me.graph.centerSelection([node], [])
      });
      sessionStorage.resultLabel = me.resultLabel;
    ...
    filterHighlightIds (data,resultNodes){
      ...
      data.forEach(function(item){
        ...
        for(var i in itemArr){
          if(itemArr[i]!='' && typeof(itemArr[i])!= 'number' && typeof(itemArr[i])!='object'&& item.nodeType!='QB_ZP_TP'){
            if(itemArr[i].indexOf(me.resultLabel) != -1){//存在这个Label
              labelObj = {
                labelId : item.id,
                children : []
              }
    ...
    /**
     * 分页点击每一页获取可叠加家族图谱名称列表
     * @function getPageTpIndex
     * @author erbing
     */
    getPageTpIndex (page) {
      var me = this;
      me.overlapData[0].pageIndex = page;
      me.getOverlapData();
    },
    /**
     * 通过选取单位获取家族图谱名称列表
    ...
    getJzmcData (flag) {
      ...
      let url = me.joinPath + 'MapSuperpositionResource/v1/SuperpositionMaps/regions';
      var params = new URLSearchParams();
    ...
    getTreeData (flag) {
      ...
      me.graph.setHighlightText(me.keyword);
      me.keyword = '';
      if(!me.tpidOverlap && me.multiStrData[0].radio.length == 0){  //智能搜索页面图谱跳转过来判断（备用）
        me.$Message.warning('请选择图谱名称！');
        return false;
      }
      if(flag){
        //sessionStorage.tpid = me.multiStrData[0].radio;
        sessionStorage.firstOverlapTpid = me.multiStrData[0].radio || me.tpidOverlap;  //第一次串并保存的图谱id
        if(!me.statusList.historyModalShow){
          me.historyData = []; //每次串并都会清空 历史记录
        }
        sessionStorage.overlapPassiveID = me.overlapPassiveID = '';
      }else{

        //me.overlapPassiveID = sessionStorage.tpid;  //不是首次通过串并的，设置overlapPassiveID 为上一次串并得到的me.multiStrData[0].radio
      }
      // if(me.multiStrData[0].radio == me.overlapPassiveID){
      //   me.overlapPassiveID = ''
      // }

      // if(me.multiStrData[0].radio == sessionStorage.overlapPassiveID){  //判断如果是同一个图谱,清空overlapPassiveID,(备用-----------------)
      //   sessionStorage.overlapPassiveID = '';
      // }
      me.highlightNodeIDs = [];
      var params = new URLSearchParams();
      let url = me.joinPath + 'MapSeriesParallelResource/v1/SeriesParallelMaps/tree/mapids';
      params.append('token', me.token);
      // params.append('passiveID', me.overlapPassiveID);
      params.append('passiveID', sessionStorage.overlapPassiveID);
      params.append('tpids',  me.multiStrData[0].radio || me.tpidOverlap);  //智能搜索页面图谱跳转过来判断（备用）
      me.statusList.d3Loading = true;
      this.$axios.post(url, params)
      .then(response => {
        me.getOverlapData();  //获取叠加的图谱数据
        //me.overlapPassiveID = me.overlapPassiveID || me.multiStrData[0].radio;  //高亮实体
        // me.tpData = response.data.tpData
        sessionStorage.overlapPassiveID = me.overlapPassiveID = me.multiStrData[0].radio || me.tpidOverlap;  //不是首次通过串并的，设置overlapPassiveID 为上一次图谱的id
        var oid = response.data.tpData.children[0].oid;//增加 根据图谱第一条数据的id, 获取图谱的相关信息
        me.commonIds = response.data.assessData[oid].commonID;
        me.highlight = response.data.highlight;
        var data = me.parseData(response.data.tpData,false);
        me.graph.disabledOverlap(true);
        me.mergeData = data;  //图谱合并连线
        me.tpData = response.data.tpData;  //图谱合并连线/高亮

        me.datas = response.data.tpData;  //
        me.multiStrData[0].setOverlapData = data;
        ...
        me.initialize(data);
        me.statusList.d3Loading = false;
        me.statusList.MultiModal = false;  //智能搜索页面图谱跳转过来隐藏（备用）
        // me.statusList.MultiModal = false;
    ...
    getOverlapData () {
      var me = this;
      let url = me.joinPath + 'MapSuperpositionResource/v1/SuperpositionMaps/passiveMapid';
      var params = new URLSearchParams();
      params.append('passiveId', me.tpidOverlap || me.multiStrData[0].radio);  //智能搜索页面图谱跳转过来判断（备用）
      params.append('token', me.token);
      params.append('pageIndex', me.overlapData[0].pageIndex);
      params.append('pageSize', 10);
      me.statusList.loadingoverLapTpmcShow = true;
      this.$axios.post(url, params)
      .then(response => {
        me.statusList.loadingoverLapTpmcShow = false;
        const data = response.data.data;
        data.forEach(item=>{
          item.tppg = item.tppg.replace('{','').replace('}','');
        })
        me.overlapData[0].AllGroup = data;
        me.overlapData[0].pageTotal = response.data.total;
      })
      .catch(error => {
        me.statusList.loadingoverLapTpmcShow = false;
        console.log(error);
     ...
     */
    getOverlapAfterData () {
      if(!this.overlapData[0].radio){
        this.$Message.warning('请选择要叠加的图谱！');
        return false;
      }
      var me = this;
      let url = me.joinPath + 'MapSuperpositionResource/v1/Superposition/link';
      var params = new URLSearchParams();
      params.append('token', me.token);
      params.append('passiveId', me.multiStrData[0].radio);  //#27:6532
      params.append('initiativeIds', me.overlapData[0].radio); //#28:5452
      me.statusList.d3Loading = true;
      this.$axios.post(url, params)
      .then(response => {
        me.statusList.d3Loading = false;
        me.statusList.MultiModal = false;
        me.statusList.ShowMultiModal = true;
        // if(!response.data.data.links.length){
        //   me.graph.disabledOverlap(true);
        //   me.commonIds = response.data.data.commonID;
        //   me.graph.load(me.multiStrData[0].setOverlapData);
        //   return;
        // }

        //图谱合并连线
        me.commonIds = response.data.commonID;
        me.parseData(me.datas,false);
        ...
        // me.multiStrData[0].radio = '';  //清空图谱名称单选选中
        // me.multiStrData[0].AllGroup = []; //清空图谱名称单选列表
        // me.overlapData[0].radio = '';  //清空可叠加图谱名称单选选中
        // me.overlapData[0].AllGroup = []; //清空可叠加图谱名称单选列表
        // me.multiStrData[0].allData.forEach(function(item){  //清空选中的单位节点
        //   item.checked = false;
        // })
        // me.$refs.tree[0].getCheckedAndIndeterminateNodes().forEach(function(item){
     ...
     */
    returnPrevTp () {
      //this.multiStrData[0].radio = sessionStorage.tpid;
      this.multiStrData[0].radio = sessionStorage.firstOverlapTpid;  //返回到首次进入的图谱，设置tid 等于首次保存的图谱id
      sessionStorage.overlapPassiveID = '';
      this.rid = [];
      this.rid.push(sessionStorage.overlapTpHighlightId); //返回到一开始进来高亮绿色那个id
      this.getTreeData(true);
    },
    /**
     * 历史记录中跳转到对应的图谱
     ...
     */
    againGetTree (id,oid) {
      this.multiStrData[0].radio = id;
      this.rid = [];
      this.rid.push(oid);  //从哪个进入图谱，绿色高亮
      this.getTreeData();
    },
    parseData(data, flag){
      var me = this;
      ...
      function changeImage(data){
        if (data.nodeType == 'QB_ZP_RY') {
          ...
          data.isShow = flag;
          // data.color = 'red';
          me.multiStrData[0].tplxList.forEach(function(item){
        ...
        if(me.commonIds.length>0 && me.commonIds.indexOf(data.oid) != -1){
          // data.color = '#fd00fc';
        ...
        if(Object.keys(me.highlight).length === 0){
          ...
          for(var i in me.highlight){
            for(var j in me.$config.project.highlightColor){
              if( i == j && me.highlight[i].length && me.highlight[i].indexOf(data.oid) != -1){
...
<style lang="less">
  ...
  .svgNT{position:absolute;bottom:30px;left:20px;border:2px solid #ccc;display:block;}
  .arc{cursor:pointer;}
