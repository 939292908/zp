 ...
 * @LastEditTime: 2019-08-15 16:22:27
...
<template>
                    ...
                    <div class="process-status">
                      <div class="upload-process">
                        ...
                        <span
                          ...
                          @click.prevent="handleReupload">重新上传</span>
                      </div>
                      <div class="upload-icon">
                        <!-- <Icon class="upload-icon-process" type="ios-loading" /> -->
                        <Icon v-if="uploadFilesStatusObj" :class="'upload-icon-' + uploadFilesStatusObj.key" :type="uploadFilesStatusObj.type" />
                      </div>
                    </div>
                  </div>
                </div>
                <input ref="input" id="fileValid" type="file" class="upload-image-file" name="file" @change="handleChange($event)">
              </label>
              <!-- 如果有错误信息显示 -->
              <div class="upload-error-box" v-if="errorContent.length > 0">
                ...
                <div class="upload-error-wrapper">
                  ...
                  <div class="error-list">
                    <div :key="index" v-if="errorContent.length>0 ? true : false" v-for="(error,index) in errorContent">
  ...
  <script>
  import { validateUploadExcel,getTypeList,templateList } from '@/api/collection/index.js'
  export default {
    name:'validateFile',
    props:{
      value:{
    ...
    methods:{
      ...
      post(file,callback) {
        ...
        validateUploadExcel(this.mapType,formData).then(res => {
          callback(res);
        }).catch(res=>{
          callback(res)
        })
  ...
  /deep/ .ivu-modal{
    ...
    .upload-image-body::-webkit-scrollbar{width:6px;height:6px;}
    .upload-image-body::-webkit-scrollbar-track{border-radius:5px;background:none;}
    .upload-image-body::-webkit-scrollbar-thumb{background-color:rgba(32,76,157,1);border-radius:5px;}
    .upload-image-body::-webkit-scrollbar-thumb:hover{background-color:rgba(45,183,245,1);}
    .upload-image-body::-webkit-scrollbar-corner{background-color:#dadada;}
    .upload-wrapper{
    ...
    .upload-error-wrapper::-webkit-scrollbar{width:6px;height:6px;}
    .upload-error-wrapper::-webkit-scrollbar-track{border-radius:5px;background:none;}
    .upload-error-wrapper::-webkit-scrollbar-thumb{background-color:rgba(32,76,157,0.4);border-radius:5px;}
    .upload-error-wrapper::-webkit-scrollbar-thumb:hover{background-color:#204C9D;}
    .upload-error-wrapper::-webkit-scrollbar-corner{background-color:#dadada;}
    .upload-image-box{
      width: 100%;
      height: 120px;
      position: relative;
