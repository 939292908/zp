 ...
 * @LastEditTime: 2019-10-28 18:14:41
 * @LastEditors: your name
 * @Description: In User Settings Edit
 * @FilePath: \map-web\src\components\MultiJoin.vue
 -->
<template>
            ...
            <div class="cont-list" v-for="(items,index) in multiStrData" :key="index" :data-index="index">
              <ul class="condition-select join">
                ...
                <li class="li family-description">
                  <span>图谱名称：</span>
                  <div class="wrapper" style="display:inline-block;margin-top: -18px;">
                    <div class="box">
                      <!-- 多选 (后期备用)-->
...
export default {
  ...
  mounted(){
    ...
    this.queryCountyTreeAuth();
  },
  methods : {
    queryCountyTreeAuth() {
      let me = this;
      comApis.queryCountyTreeAuth(this.$route.query.regions).then(res=>{
        let resgions = res.data.length > 0 ? res.data[0].allDept : ""
        me.multiStrData[0].filterData = resgions;
        this.getJzmcData(true);
      })
    },
    /**
     * 获取所有相关配置表
