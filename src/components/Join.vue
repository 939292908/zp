 ...
 * @Author: your name
 * @Date: 2019-08-13 09:20:44
 * @LastEditTime: 2019-08-13 09:20:44
  ...
  <div class="result" @click="statusList.myNavModalShow=false;statusList.uploadImg=false;statusList.personModalShow=false;statusList.newsModalShow=false;statusList.appModalShow=false;statusList.messageDetailsShow=false;statusList.guidelinesShow=false;statusList.feedbackShow=false;">
    <header> 
      <div class="logo">
        ...
        <span class="nav"><span @click="backIndex()" style="cursor:pointer;">首页></span>图谱串并</span>
      </div>
        ...
        <div class="condition-modal">
          ...
          <div class="cont-box">
            <div class="cont-list" v-for="(items,index) in multiStrData" :key="index" :data-index="index">
              <ul class="condition-select join">
                <li class="li tp-unit">
                  <span>地区单位：</span>
                  <!-- <input type="text"> -->
          ...
          <div class="cont-btn">
            ...
            <span class="sure" v-if="hasFunAuth(constantFunMap.FUN_Join_StartJoin_ID)" @click="getTreeData(true)">开始串并</span>
...
export default {
  ...
  created () {
    ...
    //if(!sessionStorage.joinHighlightId){
      sessionStorage.joinHighlightId = this.$route.query.rid || '';
    //}
  ...
  methods : {
    ...
    getTreeData (flag) {
      ...
      if(flag){
        ...
        sessionStorage.firstTpid = me.multiStrData[0].radio || me.tpidJoin;  //第一次串并保存的图谱id
        if(!me.statusList.historyModalShow){
      ...
      // if(me.multiStrData[0].radio == me.passiveID){
      //   me.passiveID = ''
      // }
      
      // if(me.multiStrData[0].radio == sessionStorage.passiveID){  //判断如果是同一个图谱,清空passiveID,(备用-----------------)
      ...
      params.append('passiveID', sessionStorage.passiveID);
      params.append('tpids', me.multiStrData[0].radio || me.tpidJoin);  //智能搜索页面图谱跳转过来判断（备用）
      me.statusList.d3Loading = true;
      this.$axios.post(url, params)
      .then(response => {
        //me.passiveID = me.passiveID || me.multiStrData[0].radio;  //高亮实体
        if(response.code == 200){
          sessionStorage.passiveID = me.passiveID = me.multiStrData[0].radio || me.tpidJoin;  //不是首次通过串并的，设置passiveID 为上一次图谱的id
    ...
    returnPrevTp () {
      ...
      this.multiStrData[0].radio = sessionStorage.firstTpid;  //返回到首次进入的图谱，设置tid 等于首次保存的图谱id
      sessionStorage.passiveID = '';  
      this.rid = [];
      this.rid.push(sessionStorage.joinHighlightId); //返回到一开始进来高亮绿色那个id
      this.getTreeData(true);
    },
    /**
     * 历史记录中跳转到对应的图谱
